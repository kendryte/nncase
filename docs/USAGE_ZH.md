...
- `--use-mse-quant-w`指定是否使用mse(mean-square error, 均方误差)来量化权重.
...
- `--preprocess`指定是否预处理
- `--swapRB`指定是否交换红和蓝两个通道数据, 用于实现RGB2BGR或BGR2RGB功能
- `--mean`指定预处理的标准化参数均值
- `--std`指定预处理的标准化参数方差
- `--input-range`指定输入数据反量化后的数据范围
- `--input-shape`指定输入数据的形状. 若与模型的输入形状不同, 则预处理时会做resize/pad等处理
