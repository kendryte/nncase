...
void onnx_importer::convert_op_DequantizeLinear(const onnx::NodeProto &node)
{
    ...
    // If both x_scale and x_zero_point are scalars, axis is ignored.
    ...
    {
        throw std::runtime_error("scale: only support per-tensor dequantize.");
    }
    ...
    else
    {
        ...
        case dt_uint8:
        {
            ...
            if (zp_value.size() > 1)
            {
                throw std::runtime_error("zero point: only support per-tensor dequantize.");
            }
        ...
        default:
        {
            throw std::runtime_error("DequantizeLinear: unsupported data type:" + std::string(datatype_names(input_type)));
        }
        }
    }

    auto op = graph_.emplace<dequantize>(input_type, input_shape, dt_float32, qparam);
    op->name(op_name);
