...
void onnx_importer::convert_op_Resize(const NodeProto &node)
{
    ...
    if (new_size.empty())
    {
        ...
        if (data)
            std::transform(std::begin(data.value()), std::end(data.value()), std::begin(input_shape), std::back_inserter(new_size),
                [](const auto axis_size, const auto scale) { return static_cast<int>(std::round(axis_size * scale)); });
    }

    const auto mode = parse_image_resize_mode(get_attribute<std::string>(node, "mode").value());
    const auto align_corners = parse_align_corners(get_attribute<std::string>(node, "coordinate_transformation_mode").value());
