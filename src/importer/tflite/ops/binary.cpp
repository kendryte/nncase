...
//
//    auto div = graph_.emplace<binary>(binary_div, get_shape(input_a.shape()),
//    get_shape(input_b.shape()), value_range<float>::full());
//    div->name(std::string(get_tensor(op.outputs(), 0).name()->string_view()) +
//    "/binary");
//
//    // input_a dequantize
...
//        auto input_a_dequant =
//        graph_.emplace<dequantize>(to_data_type(input_a.type()),
//        get_shape(input_a.shape()), dt_float32, input_a_paras);
//        input_a_dequant->name(std::string(get_tensor(op.outputs(),
//        0).name()->string_view()) + "/input_a_dequant");
...
//        auto input_b_dequant =
//        graph_.emplace<dequantize>(to_data_type(input_b.type()),
//        get_shape(input_b.shape()), dt_float32, input_b_paras);
//        input_b_dequant->name(std::string(get_tensor(op.outputs(),
//        0).name()->string_view()) + "/input_b_dequant");
...
//    floor->name(std::string(get_tensor(op.outputs(), 0).name()->string_view())
//    + "/unary"); floor->input().connect(div->output());
//
//    // output quantize
//    if (floor->output().type() != to_data_type(output.type()))
//    {
//        quant_param_t output_paras = to_quant_param(output.quantization());
//        auto output_quant = graph_.emplace<quantize>(dt_float32,
//        get_shape(output.shape()), to_data_type(output.type()), output_paras);
//        output_quant->name(std::string(get_tensor(op.outputs(),
//        0).name()->string_view()) + "/output_quant");
...
//{
//    convert_binary(op, binary_sub,
//    op.builtin_options_as_SubOptions()->fused_activation_function());
//}
//
// void tflite_importer::convert_binary(const tflite::Operator &op, binary_op_t
// binary_op, tflite::ActivationFunctionType activation)
//{
//    auto &input_a = get_tensor(op.inputs(), 0);
//    auto &input_b = get_tensor(op.inputs(), 1);
//    auto &output = get_tensor(op.outputs(), 0);
//
//    auto add = graph_.emplace<binary>(binary_op, get_shape(input_a.shape()),
//    get_shape(input_b.shape()), to_float_clamp_range(activation));
//    add->name(std::string(get_tensor(op.outputs(), 0).name()->string_view()) +
//    "/binary");
//
