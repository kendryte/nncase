    ...
    for (size_t i = 0; i < index.size(); i++)
    {
        ...
        else
        {
            ...
            if (cnt_idx > (int32_t)in_shape[i] - 1)
            {
                pad_element = true;
                if (mode == pad_reflect)
                    in_index[i] = in_shape[i] - 2 - ((size_t)cnt_idx > in_shape[i] ? (size_t)cnt_idx - in_shape[i] : in_shape[i] - (size_t)cnt_idx) % (in_shape[i] + 1);
                else if (mode == pad_symmetric)
                    in_index[i] = in_shape[i] - 1 - ((size_t)cnt_idx > in_shape[i] ? (size_t)cnt_idx - in_shape[i] : in_shape[i] - (size_t)cnt_idx) % (in_shape[i] + 1);
                else if (mode == pad_edge)
