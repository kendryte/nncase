...
result<void> kernels::reduce_window2d(reduce_op_t op, const float *input, float init_value, float *output, const runtime_shape_t &in_shape,
...
{
    return cpu::reference::reduce_window2d(op, input, init_value, output, in_shape, in_strides, out_strides, padding_h,
        padding_w, filter_h, filter_w, stride_h, stride_w, dilation_h, dilation_w, fused_activation, count_include_pad, context);
}
