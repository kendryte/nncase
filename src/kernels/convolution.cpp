    ...
    int32_t groups, int32_t stride_h, int32_t stride_w, int32_t dilation_h, int32_t dilation_w, value_range<float> fused_activation, kernel_context &context) noexcept
{
    ...
    else if (filter_h == 5 and filter_w == 5 and groups == 1 and padding_h.before == 0 and padding_h.after == 0 and padding_w.before == 0 and padding_w.after == 0 and dilation_h == 1 and dilation_w == 1)
    {
        if (stride_h == 1 and stride_w == 1)
        {
            return cpu::optimized::conv2d_3x3_s1<2, 5, 5, 1, 1>(CONV_ARGS);
        }
        else if (stride_h == 2 and stride_w == 2)
        {
            return cpu::optimized::conv2d_3x3_s1<2, 5, 5, 2, 2>(CONV_ARGS);
        }
    }
    if (filter_h == 7 and filter_w == 7 and groups == 1 and padding_h.before == 0 and padding_h.after == 0 and padding_w.before == 0 and padding_w.after == 0 and dilation_h == 1 and dilation_w == 1)
    {
        if (stride_h == 1 and stride_w == 1)
        {
            return cpu::optimized::conv2d_3x3_s1<2, 7, 7, 1, 1>(CONV_ARGS);
        }
        else if (stride_h == 2 and stride_w == 2)
        {
            return cpu::optimized::conv2d_3x3_s1<2, 7, 7, 2, 2>(CONV_ARGS);
        }
    }
    // general conv
