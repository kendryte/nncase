...
namespace NnCase.Targets.CPU.IR.Operators
{
    public class CPUConv2D : Node
    {
        ...
        public CPUConv2D(Shape inputShape, DenseTensor<float> weights, DenseTensor<float> bias, Padding paddingH, Padding paddingW, int strideH, int strideW, int dilationH, int dilationW, ValueRange<float> fusedActivation)
        {
            Weights = weights;
            Bias = bias;
            PaddingH = paddingH;
