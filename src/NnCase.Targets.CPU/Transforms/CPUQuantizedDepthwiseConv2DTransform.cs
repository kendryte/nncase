...
{
    public class CPUQuantizedDepthwiseConv2DTransform : Transform
    {
        private readonly Quantizer _quantizer;

        protected override bool SkipSelfContainedCheck => true;

        public CPUQuantizedDepthwiseConv2DTransform(Quantizer quantizer)
        {
        ...
        protected override bool OnTryMatch(Node node, TransformContext context)
        {
            if (node is FakeQuantize q
                && NodeTreeHelper.TryGetDirectChild<CPUDepthwiseConv2D>(q, out var conv2d)
        ...
        public override void Process(TransformContext context)
        {
            ...
            var oldConv2D = (CPUDepthwiseConv2D)context.MatchedNodes[1];
            ...
            var conv2d = context.Graph.AddNode(new CPUQuantizedDepthwiseConv2D(q.Output.Shape, qWeights, qBias, oldConv2D.PaddingH, oldConv2D.PaddingW, oldConv2D.StrideH, oldConv2D.StrideW, oldConv2D.DilationH, oldConv2D.DilationW, iqParam.ZeroPoint, wqParam.ZeroPoint, oMul.RoundedMul, oMul.Shift, yqParam.ZeroPoint));
        ...
        private DenseTensor<int> QuantizeBias(CPUDepthwiseConv2D conv2d, float sa)
        {
        ...
        private (QuantizationParam wqParam, DenseTensor<byte> qWeights) QuantizeWeights(CPUDepthwiseConv2D conv2d)
        {
