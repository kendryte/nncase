...
internal sealed class GtkSynchronizationContext : SynchronizationContext
{
    ...
    private GtkSynchronizationContext(State state)
    {
        _state = state;
    }
    ...
    {
        var completion = new SynchronizationTaskCompletionSource<Action, object?>(action);
        ExecuteSynchronouslyIfPossible(
            state =>
            {
                var completion = (SynchronizationTaskCompletionSource<Action, object?>)state!;
                try
                {
                    completion.Callback();
    ...
    {
        var completion = new SynchronizationTaskCompletionSource<Func<Task>, object?>(asyncAction);
        ExecuteSynchronouslyIfPossible(
            async (state) =>
            {
                var completion = (SynchronizationTaskCompletionSource<Func<Task>, object?>)state!;
                try
                {
    ...
    {
        var completion = new SynchronizationTaskCompletionSource<Func<TResult>, TResult>(function);
        ExecuteSynchronouslyIfPossible(
            (state) =>
            {
                var completion = (SynchronizationTaskCompletionSource<Func<TResult>, TResult>)state!;
                try
                {
    ...
    {
        var completion = new SynchronizationTaskCompletionSource<Func<Task<TResult>>, TResult>(asyncFunction);
        ExecuteSynchronouslyIfPossible(
            async (state) =>
            {
                var completion = (SynchronizationTaskCompletionSource<Func<Task<TResult>>, TResult>)state!;
                try
                {
    ...
    {
        return new GtkSynchronizationContext(_state);
    }
    ...
    private Task Enqueue(Task antecedent, SendOrPostCallback d, object? state, bool forceAsync = false)
    {
        ...
        return antecedent.ContinueWith(
            BackgroundWorkThunk,
            new WorkItem(this, executionContext!, d, state!),
    ...
    private class State
    {
        public bool IsBusy { get; set; } // Just for debugging
    ...
    private record WorkItem(GtkSynchronizationContext SynchronizationContext, ExecutionContext ExecutionContext, SendOrPostCallback Callback, object State);

    private class SynchronizationTaskCompletionSource<TCallback, TResult> : TaskCompletionSource<TResult>
    {
        public SynchronizationTaskCompletionSource(TCallback callback)
        {
            Callback = callback;
        }
