...
public interface IMatchResult : IEnumerable<KeyValuePair<IPattern, object>>
{
    ...
    /// <param name="pattern">Pattern.</param>
    /// <returns>Match result.</returns>
    T Get<T>(IPattern<T> pattern) => (T)this[pattern];
    ...
    (T1 Result1, T2 Result2) Get<T1, T2>(IPattern<T1> pattern1, IPattern<T2> pattern2)
    ...
    /// <returns>Match results.</returns>
    (T1 Result1, T2 Result2, T3 Result3) Get<T1, T2, T3>(IPattern<T1> pattern1, IPattern<T2> pattern2, IPattern<T3> pattern3)
