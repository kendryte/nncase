...
public class UnitTestBroadcastMarker : TransformTestBase
{
    ...
    public void TestBroadcastInputMarker()
    {
        ...
        var a = IR.F.Math.MatMul(
            Reshape(new Marker(WellknownMarkerNames.RangeOf, (Expr)input, (Expr)new[] { -1f, 1f }), input.Shape),
            Reshape(new Marker(WellknownMarkerNames.RangeOf, (Expr)input, (Expr)new[] { -2f, 2f }), input.Shape));
    ...
    {
        var input = Testing.Rand<float>(1, 3, 24, 24);
        var a = new Marker(WellknownMarkerNames.RangeOf, Reshape(Abs(input), input.Shape), (Expr)new[] { -1f, 1f });
