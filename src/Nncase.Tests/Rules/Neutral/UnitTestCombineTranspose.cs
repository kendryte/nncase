...
public class UnitTestCombineTranspose : TransformTestBase
{
    public static readonly TheoryData<BinaryOp, int[], int[], int[], bool> CombineTransposeConstBinaryPositiveData = new()
    {
        ...
        { BinaryOp.Add, new[] { 1, 32, 32, 64, }, new[] { 1 }, new[] { 0, 3, 1, 2 }, false },
        ...
        { BinaryOp.Div, new[] { 1 }, new[] { 1, 32, 32, 64, }, new[] { 0, 3, 1, 2 }, true },
    ...
    {
        Expr lhs = leftConst ?
          lShape.Sum() == 1 ? 0.5f : Const.FromValue(Random.Normal(DataTypes.Float32, 0, 1, 3, lShape).Evaluate()) :
