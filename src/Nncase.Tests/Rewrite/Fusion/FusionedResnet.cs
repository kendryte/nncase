...
internal sealed class ResNet
{
    ...
    )
    {
        ...
        var conv1_creator = (Expr[] inputs) =>
        {
            ...
            return new Fusion("Conv3x3Fusion", Callable.StackVMModuleKind, IR.F.NN.Conv2D(v_input, weights, bias, new[] { 2, 2 }, new[,] { { 3, 3 }, { 3, 3 } }, new[] { 1, 1 }, PadMode.Constant, groups), new[] { v_input });
        };
