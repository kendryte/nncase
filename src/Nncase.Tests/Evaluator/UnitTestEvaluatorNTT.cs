...
public sealed class UnitTestEvaluatorNTT : TestClassBase
{
    ...
    public void TestIm2colConv(bool vectorize, long[] inputShape, long[] weightShape, int[] padding, int[] strides)
    {
        ...
        Expr post;
        {
            if (vectorize)
            {
                var col = IR.F.NTT.Im2col(IR.F.Tensors.Vectorize(input, new[] { 4 }, new[] { 1 }), new[] { weightShape[2], weightShape[3] }, strides, padding, new[] { 1 }, new[] { 0 });
                var newW = IR.F.Tensors.Reshape(IR.F.Tensors.Vectorize(weights, new[] { 4 }, new[] { 1 }), new[] { weightShape[0], weightShape[1] / 4 * weightShape[2] * weightShape[3] });
