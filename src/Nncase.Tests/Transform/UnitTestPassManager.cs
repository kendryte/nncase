...
public sealed class UnitTestPassManager : TestClassBase
{
    ...
    public void TestPassMangerUpdateDependence2()
    {
        ...
        */
        ...
        var prim_func_0 = T.PrimFunc("prim_func_0", "k?", T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 24, 32, 3 }), out var _), T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _)).Body(
          T.Nop()).Build();
        var func_0 = new PrimFunctionWrapper(prim_func_0, 1);

        var prim_func_1 = T.PrimFunc("prim_func_1", "k?", T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _), T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _)).Body(
          T.Nop()).Build();
        var func_1 = new PrimFunctionWrapper(prim_func_1, 1);

        var prim_func_2 = T.PrimFunc("prim_func_2", "k?", T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _), T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 23, 30, 16 }), out var _)).Body(
          T.Nop()).Build();
        ...
        // prim_func_2 for update
        var prim_func_1_update = T.PrimFunc("prim_func_1_update", "k?", T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _), T.CreateBufferVar(new(DataTypes.Float32, new[] { 1, 3, 24, 32 }), out var _)).Body(
          T.Nop(),
          T.Nop()).Build();
