...
public sealed class UnitTestBFloat16Converters
{
    public static unsafe IEnumerable<object[]> TestBoolData =>
        new[]
        {
            new object[] { new bool[] { true, false, true, false }, new float[] { 1, 0, 3, 0 }.Select(x => (BFloat16)x).ToArray<BFloat16>(), CastMode.KDefault },
            new object[] { new bool[] { true, false, true, false }, new float[] { 1, 0, 3, 0 }.Select(x => (BFloat16)x).ToArray<BFloat16>(), CastMode.CheckOverflow },
            new object[] { new bool[] { true, false, true, false }, new float[] { 1, 0, 3, 0 }.Select(x => (BFloat16)x).ToArray<BFloat16>(), CastMode.Reinterpret },
        };

    [Theory]
    [MemberData(nameof(TestBoolData))]
    public void TestConvertToBool(bool[] expected, BFloat16[] src, CastMode mode)
    {
        var actual = new bool[src.Length];
        var c = new BFloat16Converters();
    ...
    public void TestConvertToBoolException()
    {
        ...
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (byte)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (short)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (ushort)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, (float)ushort.MaxValue + 1 };
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (int)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (uint)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (long)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    {
        var f = new float[] { 1, 2, 3, 4 };
        var expected = f.Select(x => (ulong)x).ToArray();
        var a = f.Select(x => (BFloat16)x).ToArray();
    ...
    public void TestConvertToHalf()
    {
        ...
        var c = new BFloat16Converters();
        c.ConvertTo(a, actual, CastMode.KDefault);
        Assert.Equal(f.Select(x => (Half)x).ToArray(), actual);
    }
    ...
    public void TestConvertToDouble()
    {
        ...
        var c = new BFloat16Converters();
        c.ConvertTo(a, actual, CastMode.KDefault);
        Assert.Equal(f.Select(x => (double)x).ToArray(), actual);
    }
    ...
    public void TestConvertToBFloat16Exception()
    {
        var f = new float[] { 1, 2, 3, 4 };
        var a = f.Select(x => (BFloat16)x).ToArray();
        var actual = new BFloat16[a.Length - 1];
        var c = new BFloat16Converters();
