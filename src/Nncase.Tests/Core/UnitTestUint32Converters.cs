...
public sealed class UnitTestUInt32Converters
{
    public static unsafe IEnumerable<object[]> TestBoolData =>
        new[]
        {
            new object[] { new bool[] { true, false, true, false }, new uint[] { 1, 0, 3, 0 }, CastMode.KDefault },
            new object[] { new bool[] { true, false, true, false }, new uint[] { 1, 0, 3, 0 }, CastMode.CheckOverflow },
            new object[] { new bool[] { true, false, true, false }, new uint[] { 1, 0, 3, 0 }, CastMode.Reinterpret },
        };

    [Theory]
    [MemberData(nameof(TestBoolData))]
    public void TestConvertToBool(bool[] expected, uint[] src, CastMode mode)
    {
        var actual = new bool[src.Length];
        var c = new UInt32Converters();
    ...
    {
        var a = new uint[] { 1, 2, 3, uint.MaxValue };
    ...
    {
        var a = new uint[] { 1, 2, 3, 4 };
        var actual = new uint[a.Length - 1];
        var c = new UInt32Converters();
