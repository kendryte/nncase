...
public sealed class UnitTestUInt8Converters
{
    public static unsafe IEnumerable<object[]> TestBoolData =>
        new[]
        {
            new object[] { new bool[] { true, false, true, false }, new byte[] { 1, 0, 3, 0 }, CastMode.KDefault },
            new object[] { new bool[] { true, false, true, false }, new byte[] { 1, 0, 3, 0 }, CastMode.CheckOverflow },
            new object[] { new bool[] { true, false, true, false }, new byte[] { 1, 0, 3, 0 }, CastMode.Reinterpret },
        };

    [Theory]
    [MemberData(nameof(TestBoolData))]
    public void TestConvertToBool(bool[] expected, byte[] src, CastMode mode)
    {
        var actual = new bool[src.Length];
        var c = new UInt8Converters();
    ...
    {
        var a = new byte[] { 1, 2, 3, byte.MaxValue };
    ...
    {
        var a = new byte[] { 1, 2, 3, 4 };
        var actual = new byte[a.Length - 1];
        var c = new UInt8Converters();
