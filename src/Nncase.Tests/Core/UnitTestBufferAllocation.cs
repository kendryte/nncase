...
public sealed class UnitTestBufferAllocation
{
    ...
    {
        var bufferAllocation = new BufferAllocation(default, DataTypes.Int8, 1UL, 1UL, 1UL, [1, 3, 16, 16], [1, 1, 1, 1], [1]);
        ...
        Assert.Equal([1, 3, 16, 16], bufferAllocation.Shape);
        Assert.Equal([1, 1, 1, 1], bufferAllocation.Strides);
        Assert.Equal([1], bufferAllocation.StridesShape);
        ...
        Assert.Throws<KeyNotFoundException>(() => new BufferAllocation(default, DataTypes.Utf8Char, 1UL, 1UL, 1UL, [1, 3, 16, 16], [1, 1, 1, 1], [1]).MemoryRange);
    }
