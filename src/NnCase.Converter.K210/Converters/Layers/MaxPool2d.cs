...
namespace NnCase.Converter.K210.Converters.Layers
{
    ...
    public class MaxPool2dConverter
    {
        ...
        public MaxPool2dLayerArgument DeserializeBin(int offset, K210BinDeserializeContext context)
        {
            var sr = context.GetReaderAt(offset);
            var argument = new MaxPool2dLayerArgument
            {
        ...
        public void Forward(MaxPool2dLayerArgument argument, ForwardContext context)
        {
                    ...
                    for (int ox = 0; ox < argument.OutputWidth; ox++)
                    {
                        ...
                        for (int ky = kernelYStart; ky < kernelYEnd; ky++)
                        {
                            for (int kx = kernelXStart; kx < kernelXEnd; kx++)
                            {
                                ...
                                value = Math.Max(value, channelSrc[inY * (int)argument.InputWidth + inX]);
                            }
                        }
