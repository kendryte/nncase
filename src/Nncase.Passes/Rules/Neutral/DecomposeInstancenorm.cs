...
public sealed partial class DecomposeInstanceNorm : IRewriteRule
{
    ...
    private Expr? GetReplace(Expr input, Call call, InstanceNormalization instanceNorm, TensorConst scale, TensorConst bias, TensorConst eps)
    {
        ...
        var newScale = Tensor.FromBytes(scale.CheckedDataType, scale.Value.BytesBuffer.ToArray(), new[] { scale.CheckedShape[0].FixedValue }.Concat(Enumerable.Repeat(1L, axis.Length).ToArray()).ToArray());
        var newBias = Tensor.FromBytes(bias.CheckedDataType, bias.Value.BytesBuffer.ToArray(), new[] { bias.CheckedShape[0].FixedValue }.Concat(Enumerable.Repeat(1L, axis.Length).ToArray()).ToArray());
