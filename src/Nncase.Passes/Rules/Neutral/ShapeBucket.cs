...
public class BucketFusion : Fusion
{
    ...
    public BucketFusion(string name, string moduleKind, Expr body, ReadOnlySpan<Var> parameters, Var[] effectVar) : base(
        name, moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }

    public BucketFusion(string moduleKind, Expr body, ReadOnlySpan<Var> parameters, Var[] effectVar) : base(moduleKind,
        body,
        parameters)
    {
        EffectVar = effectVar;
    ...
    public BucketFusion(string name, string moduleKind, Var[] effectVar, Expr body, params Var[] parameters) : base(name,
        moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }

    public BucketFusion(string moduleKind, Var[] effectVar, Expr body, params Var[] parameters) : base(moduleKind, body,
        parameters)
    {
        EffectVar = effectVar;
    }

    public BucketFusion With(string? name = null, string? moduleKind = null, Expr? body = null, Var[]? parameters = null)
...
internal static class ShapeBucketHelper
{
    ...
    public static Var[] MakeEffectVarArray(Dictionary<Var, Expr[]> varMap, string relPath, params Expr[] args)
    {
...
public partial class MarkerCallToFusion<T> : RewriteRule<Pattern> where T : Op
{
    ...
    public string Name => CurrentCall.Target.GetType().Name;
    ...
    public Expr GetReplace(Marker callMarker)
    {
        ...
        var set = MakeEffectVarArray(varMap, RelPath, args);
...
public partial class FusionBucket : RewriteRule<Pattern>
{
    ...
    public override Pattern Pattern => IsCall("outerCall",
        IsFusion("fusion", "stackvm",
    ...
    public Expr FixInput(Expr body, int[][] shapeList, Var[] fusionVars, Expr[] outerArgs)
    {
        ...
        DumpIR(result, "FixInput", relPath);
        return result;
    }

    public Expr? GetReplace(Call outerCall, BucketFusion fusion, Expr fusionBody)
    {
        Console.WriteLine($"FusionBucketGetReplace {counter} {fusion.Name}");
        relPath = $"{counter}";
        DumpIR(outerCall, $"BucketOriginFusion_{fusion.Name}", relPath);
        ...
        using var _ = new ExprPinner(fusionInputsShapeExpr.Values.SelectMany(x => x).ToArray());
        ...
        PrintMinMaxShape(minFixedShapeList, maxFixedShapeList, relPath);
        ...
        {
            DumpIR(fusionBody, "BucketResultFix", relPath);
        ...
        var body = Split(fusionBody, fusionVars, info, 0, 1, dimVarValues, args, varMap,
            fusionInputsShapeExpr, fusionInputShapes);
        body.InferenceType();
        // FixInput Replace Var
        var newBody = ReplaceFusionVarWithCallArgs(fusion, args, body);
        DumpIR(newBody, "BucketResult", relPath);
        counter++;

        var minFixedShapeList1 = ComputeFixedShape(new[]{body}, minDict, varMap, fusionInputsShapeExpr);
        var maxFixedShapeList1 = ComputeFixedShape(new[]{body}, maxDict, varMap, fusionInputsShapeExpr);
        // PrintMinMaxShape(minFixedShapeList1, maxFixedShapeList1, relPath);
        return newBody;
    }
    ...
    private Expr Split(Expr fusionBody, Var[] fusionVars, SegmentInfo info, int current,
        int limit, Dictionary<Var, int[]> varValues, Expr[] fusionInputs, Dictionary<Var, Expr[]> varMap,
        Dictionary<Var, Expr[]> FusionInputData,
        ...
        var body = segments.OrderByDescending(x => x).Aggregate(
            ...
            (sum, seg) =>
            {
                ...
                var thenBody = current + 1 < limit
                    ? Split(fusionBody, fusionVars, info, current + 1, limit, varValues, fusionInputs,
                        varMap, FusionInputData, FusionInputsShape)
                    : MakeSplitEntry(fusionBody, fusionVars, varMap, varInfo, FusionInputData,
                        FusionInputsShape, fusionInputs, relPath, seg);
                var elseBody = sum;
                i++;
    ...
    public static Expr MakeSplitEntry(Expr originBody, Var[] fusionVars,
        Dictionary<Var, Expr[]> inputInfo,
        Dictionary<Var, IValue> varInfo, Dictionary<Var, Expr[]> fusionInputdata,
        Dictionary<Var, Expr[]> fusionInputsShape, Expr[] fusionInputs, string relPath, int seg)
    {
        ...
        call = fusionVars.Select(v => newVars.FindFirst(newVar => newVar.Name == v.Name)).Zip(fixInputs).Aggregate(call,
            (sum, pair) =>
            {
    ...
    public static Expr PreProcess(Var input, Dictionary<Var, Expr[]> inputInfo,
    ...
    public static int[] ShapeEvaluate(Expr expr, Dictionary<Var, Expr[]> info, Dictionary<Var, IValue> varInfo,
        Dictionary<Var, Expr[]> fusionInfo, string relPath = "", int i = 0)
    {
    ...
    private int[][] ComputeFixedShape(Expr[] fusionVars, Dictionary<Var, IValue> varInfo, Dictionary<Var, Expr[]> varMap,
        Dictionary<Var, Expr[]> FusionInputData) =>
        fusionVars.Select((arg, i) =>
        {
            var fixedShape = ShapeEvaluate(arg, varMap, varInfo, FusionInputData, relPath, i);
            return fixedShape;
        }).ToArray();
        ...
        Dictionary<Var, Expr[]> inputInfo, ShapeBucketOptions options)
    {
        ...
        // DimVarName -> Dict.key -> Dict.Value
        var minDict = options.RangeInfo.ToDictionary(pair => vars.FindFirst(v => v.Name == pair.Key),
            pair => (IValue)Value.FromTensor(pair.Value.Item1));
        var maxDict = options.RangeInfo.ToDictionary(pair => vars.FindFirst(v => v.Name == pair.Key),
            pair => (IValue)Value.FromTensor(pair.Value.Item2));
    ...
    private Dictionary<Var, Expr[]> MakeFusionInputShapeExpr(Call call, BucketFusion fusion,
...
public partial class MergeNextMarkerToFusion : MergeFusionBase
{
    ...
    public Expr GetReplace(Marker marker, Call fusionOuterCall, BucketFusion fusion, RunPassContext context)
    {
        ...
        var result = marker.With(target: fusionOuterCall.With(target: fusion.With(body: marker.With(target: fusion.Body))));
...
{
    public override Pattern Pattern => IsCall("fusionOuterCall",
        IsFusion(
            "fusion",
            "stackvm",
            IsWildcard(),
...
{
    public Pattern FusionCall => IsCall("fusionOuterCall",
        IsFusion(
            "fusion",
            "stackvm",
            IsWildcard(),
    ...
    public override Pattern Pattern => IsCallWildcard("nextCall",
        IsWildcard("target"),
        IsAlt("maybeFusionCallMarker", FusionCall,
    ...
    public Expr GetReplace(Call nextCall, Expr maybeFusionCallMarker, Expr target, Call fusionOuterCall,
        BucketFusion fusion)
    {
    ...
    {
        var array = MakeEffectVarArray(CompileSession.CompileOptions.ShapeBucketOptions.VarMap, null,
...
public partial class MergePrevCallToFusion : MergeFusionBase
{
    ...
    public Expr GetReplace(Call fusionOuterCall, BucketFusion fusion, Call lhsPrevCall, Expr lhsTarget, Expr lhsArg)
    {
        ...
        var newArgs = ReplaceItems(fusionOuterCall.Arguments.ToArray(),
    ...
    private (Call[], Expr[]) CollectInputsInfo(Call fusionOuterCall)
    {
    ...
    private (Call[], Expr[]) CollectInputsInfo(Call fusionOuterCall, Call lhsPrevCall, Expr lhsArg)
    {
