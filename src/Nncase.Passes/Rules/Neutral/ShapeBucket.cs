...
public class VarFusion : Fusion
{
    ...
    public VarFusion(string name, string moduleKind, Expr body, ReadOnlySpan<Var> parameters, Var effectVar) : base(name, moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }

    public VarFusion(string moduleKind, Expr body, ReadOnlySpan<Var> parameters, Var effectVar) : base(moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }

    public VarFusion(string name, string moduleKind, Var effectVar, Expr body, params Var[] parameters) : base(name, moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }

    public VarFusion(string moduleKind, Var effectVar, Expr body, params Var[] parameters) : base(moduleKind, body, parameters)
    {
        EffectVar = effectVar;
    }
}
...
public partial class MatmulToFusion : IRewriteRule
{
    ...
    public Expr? GetReplace(Expr arg0, Expr arg1)
    {
        ...
        visitor.Visit(arg0.EvaluateShapeExpr(VarMap));
        visitor.Visit(arg1.EvaluateShapeExpr(VarMap));
...
public partial class ReplaceRewrite : IRewriteRule
{
    ...
    private int[][] ComputeFixedShape(Call call, Dictionary<Var, IValue> varInfo) =>
        ...
        {
            var fixedShape = ShapeEvaluate(arg, InputInfo, varInfo, FusionInputData);
            return fixedShape;
        }).ToArray();
    ...
    private Expr Split(Call call, int inputIndex, int dimIndex, int current, int limit, int[] segments)
    {
        ...
        var body = segments.OrderByDescending(x => x).Aggregate(
            ...
            (sum, seg) =>
            {
                ...
                var varInfo = new Dictionary<Var, IValue> { { DimVar, Value.FromTensor((long)seg) } };
...
public partial class FusionBucket : IRewriteRule
{
    ...
    public Expr? GetReplace(Call call, VarFusion fusion)
    {
        ...
        var newBody = CompilerServices.Rewrite(fusion.Body, new[] { new ReplaceRewrite(fusion.EffectVar, InputInfo, fusionInputData) }, ctx);
        // unpack fusion
