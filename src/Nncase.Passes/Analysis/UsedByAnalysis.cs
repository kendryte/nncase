...
namespace Nncase.Passes.Analyses;
...
internal sealed class UsedByAnalysisVisitor : ExprVisitor<bool, IRType>
{
    ...
    protected override bool DispatchVisit(Expr expr)
    {
        if (_entry is null && expr is BaseFunction baseFunction)
        {
            _entry = baseFunction;
    ...
    protected override bool DefaultVisitLeaf(Expr expr) => false;
