...
namespace NnCase.Converter.Transforms.K210
{
    public class K210SeprableConv2dTransform : Transform
    {
        protected override bool OnTryMatch(Layer layer, TransformContext context)
        {
            try
            {
                if (layer is DepthwiseConv2d dwConv2d)
                {
                    ...
                    foreach (var nextLayer in dwConv2d.Output.Connections.Select(o => o.To.Owner))
                    {
        ...
        public override void Process(TransformContext context)
        {
            ...
            var conv2d = (Conv2d)context.MatchedLayers[1];
            var input = dwConv2d.Input.Connection.From;
            var output = conv2d.Output;

            dwConv2d.Input.ClearConnection();

            var newDwConv2d = new DepthwiseConv2d(input.Dimensions, dwConv2d.Weights, dwConv2d.Bias, Padding.Valid, 1, 1, dwConv2d.FusedActivationFunction);
