...
{
    public class EliminateTwoReshapeTransform : Transform
    {
        protected override bool OnTryMatch(Layer layer, TransformContext context)
        {
            try
            {
                if (layer is Reshape reshape1)
                {
                    ...
                    {
                        if (nextLayer is Reshape reshape2 && reshape1.Input.Dimensions.SequenceEqual(reshape2.Output.Dimensions))
                        {
        ...
        public override void Process(TransformContext context)
        {
            ...
            var reshape2 = (Reshape)context.MatchedLayers[1];
