...
result<void> stackvm_runtime_function::visit([[maybe_unused]] const tensor_custom_call_op_t &op) noexcept {
    std::vector<value_t> inputs;
    for (size_t i = 0; i < op.input_size; i++) {
      try_var(input, pop_value());
      inputs.push_back(input);
    }
    try_var(output, kernels::stackvm::custom_call(op.registered_name, op.fields, inputs, nullptr, module().kernel_context()));
    dump_output(output);
    return stack_.push(std::move(output));
}
