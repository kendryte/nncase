...
result<void> stackvm_runtime_function::visit([[maybe_unused]] const tensor_bucket_slice_op_t &op) noexcept {
    dump_op(op);
    try_var(input, pop_value());
    dump_input(input);
    try_var(origin_input, pop_value());
    dump_input(origin_input);
    try_var(output, kernels::stackvm::bucket_slice(input, origin_input, nullptr, module().kernel_context()));
    dump_output(output);
    stack_.push(std::move(output));
    return ok();
}
