...
result<void> stackvm_runtime_function::visit([[maybe_unused]] const tensor_grid_sample_op_t &op) noexcept {
    dump_op("grid_sample");
    try_var(input, pop_value());
    dump_input(input);
    try_var(grid, pop_value());
    dump_input(grid);
    try_var(output, kernels::stackvm::grid_sample(op.align_corners, op.mode, op.padding_mode, input, grid, nullptr, module().kernel_context()));
    dump_output(output);
    stack_.push(std::move(output));
    return ok();
}
