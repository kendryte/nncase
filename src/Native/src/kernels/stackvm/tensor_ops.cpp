...
result<value_t> nncase::kernels::stackvm::reshape(
    ...
    [[maybe_unused]] auto s = in_tensor->buffer().size_bytes();
    [[maybe_unused]] auto new_strides = get_default_strides(shape_value);
    [[maybe_unused]] auto new_bytes =
        get_bytes(in_tensor->dtype(), shape_value, new_strides);
    auto node =
...
result<value_t>
nncase::kernels::stackvm::shape_of([[maybe_unused]] value_t input,
                                   [[maybe_unused]] value_t output,
                                   [[maybe_unused]] kernel_context &context) {
    try_var(in_tensor, input.as<tensor>());
...
result<value_t> nncase::kernels::stackvm::slice(value_t input, value_t begins,
    ...
    auto out_shape =
        slice_infer_shape(begin_values, end_values, strides_values);
