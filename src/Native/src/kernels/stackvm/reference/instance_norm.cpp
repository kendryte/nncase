...
result<void> nncase::kernels::stackvm::reference::instance_norm(
    ...
    auto run_reduce = [&](auto &&input, auto &&output, auto &&in_shape, auto &&in_strides) -> result<void> {
    ...
    try_(reference::binary(
        ...
        OUT_CAST(gsl::byte, sub_output.get()), in_shape, in_strides, tmp_out_shape, tmp_out_strides,
        sub_out_shape, sub_out_strides));
    try_(reference::unary(dt_float32, unary_op_t::square, IN_CAST(gsl::byte, sub_output.get()),
    ...
    try_(run_reduce(square_output.get(), var.get(), sub_out_shape, sub_out_strides));
