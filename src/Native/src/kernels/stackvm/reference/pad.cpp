...
template <class T>
result<void> pad_impl(const T *input, T *output, gsl::span<const size_t> in_shape,
...
interior_pad_impl(const T *input, T *output,
                  NNCASE_UNUSED gsl::span<const size_t> in_shape, gsl::span<const size_t> out_shape,
...
result<void> nncase::kernels::stackvm::reference::pad(
    ...
    gsl::span<const size_t> out_strides, const paddings_t &paddings, pad_mode_t mode,
