...
template result<void> reference::layernorm<float>(const float *input, float *output, float *scale, float *bias, const dims_t &in_shape, int32_t axis, float epsilon) noexcept;

template <typename T>
result<void> reference::layernorm(const T *input, T *output, T *scale, T *bias, const dims_t &in_shape, int32_t axis, float epsilon) noexcept
{
   auto outer_size = 1;
   auto inner_size = 1;
