    ...
    kernel_context &context) noexcept
{
    switch (op)
    {
        REDUCE_WINDOW2D_IMPL(reduce_op_t::mean, std::plus<float>(), [](float v, int32_t block_size) { return v / (float)block_size; });
        REDUCE_WINDOW2D_IMPL_NO_POST(reduce_op_t::min, [](float a, float b) { return std::min(a, b); });
        REDUCE_WINDOW2D_IMPL_NO_POST(reduce_op_t::max, [](float a, float b) { return std::max(a, b); });
...
dims_t infer_shape(const dims_t& in_shape, const dims_t& filter, const dims_t& stride,
                   const dims_t& dilation, const paddings_t& paddings) {
    auto new_shape = in_shape;
    new_shape[2] = kernels::detail::get_windowed_output_size(in_shape[2], filter[0], stride[0], dilation[0], paddings[0]);
    new_shape[3] = kernels::detail::get_windowed_output_size(in_shape[3], filter[1], stride[1], dilation[1], paddings[1]);
    return new_shape;
}
...
result<value_t> nncase::kernels::stackvm::reduce_window2d(
    ...
    value_t stride, value_t padding, value_t dilation, [[maybe_unused]] value_t ceil_mode,
    [[maybe_unused]] value_t count_include_pad, value_t output, kernel_context &context) {
