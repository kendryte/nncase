...
{
template <class T>
result<void> batch_to_space_impl(const T *input, T *output, const dims_t &in_shape, const dims_t &block_shape,
    const paddings_t &crops, const strides_t &in_strides, const strides_t &out_strides,
...
result<void> batch_to_space_impl(datatype_t type, const gsl::byte *input, gsl::byte *output, const dims_t &in_shape,
    const dims_t &block_shape, const paddings_t &crops, const strides_t &in_strides, const strides_t &out_strides,
    NNCASE_UNUSED kernel_context &context) noexcept
{
...
dims_t infer_shape(const dims_t& in_shape, const dims_t& block_shape, const paddings_t& crops) {
    auto new_shape = dims_t(in_shape.size() + 2);
    new_shape[0] = in_shape[0];
    for (int i = 0; i < 2; ++i) {
        new_shape[i + 1] = in_shape[1 + i] * block_shape.size() - crops[i].sum();
    }
    for (int i = 0; i < in_shape.size() - 1; ++i) {
        new_shape[3 + i] = in_shape[i + 1];
    }
    return new_shape;
}

result<value_t> kernels::stackvm::batch_to_space(value_t input, value_t block_shape,
                                                value_t crops, value_t output, kernel_context &context) {
    try_input(input_mem, input);
    try_var(block_shape_value, value_as_dims(block_shape));
    try_var(crops_value, value_as_paddings(crops));
    auto dtype = input_tensor->dtype();
    auto out_shape = infer_shape(input_tensor->shape(), block_shape_value, crops_value);
    try_output(out_mem, output, dtype, input_tensor->shape());
    try_(batch_to_space_impl(dtype, input_mem, out_mem,
                             input_tensor->shape(), block_shape_value, crops_value,
                             input_tensor->strides(), output_tensor->strides(), context));
    return ok(output);
}
