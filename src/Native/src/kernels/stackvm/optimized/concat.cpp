...
void _concat_contiguous_dim_copy(const dims_t &in_shape, dims_t &in_index, Callable &&line_copy, std::true_type)
{
    ...
    {
        in_index[CurAxis] = i;
        _concat_contiguous_dim_copy<Axis, CurAxis + 1>(in_shape, in_index, std::forward<Callable>(line_copy),
...
result<void> concat_contiguous_impl(gsl::span<const gsl::byte *const> inputs, T *output, const dims_t &out_shape,
    gsl::span<const dims_t> &in_strides, NNCASE_UNUSED const dims_t &out_strides, size_t axis, const dims_t &concat_dims, NNCASE_UNUSED kernel_context &context) noexcept
{
    ...
    if (axis == 0)
    {
        _concat_contiguous_dim_copy<0>(in_shape, in_index, line_copy, std::false_type {});
    }
    else if (axis == 1)
    {
        _concat_contiguous_dim_copy<1>(in_shape, in_index, line_copy, std::true_type {});
    }
    else if (axis == 2)
    {
        _concat_contiguous_dim_copy<2>(in_shape, in_index, line_copy, std::true_type {});
    }
    else if (axis == 3)
    {
        _concat_contiguous_dim_copy<3>(in_shape, in_index, line_copy, std::true_type {});
    }
    else
    {
        assert(false);
...
void dim_n_for(const dims_t &in_shape, dims_t &in_index, dims_t &out_index, Callable &&callable, std::true_type)
{
    for (size_t channel = 0; channel < in_shape[StartIndex]; ++channel)
    {
        ...
        dim_n_for<N, StartIndex + 1>(in_shape, in_index, out_index, std::forward<Callable>(callable), is_not_equal<N, StartIndex + 1>);
    }
}
...
result<void> concat_impl(gsl::span<const gsl::byte *const> inputs, T *output, const dims_t &out_shape,
    gsl::span<const dims_t> &in_strides, const dims_t &out_strides, size_t axis, const dims_t &concat_dims, NNCASE_UNUSED kernel_context &context) noexcept
{
    ...
    if (dims == 1)
    {
        dim_n_for<0>(in_shape, in_index, out_index, concat_last_dim, std::false_type {});
    }
    else if (dims == 2)
    {
        if (axis == 0)
        ...
        {
            dim_n_for<1>(in_shape, in_index, out_index, concat_last_dim, std::true_type {});
        }
    }
    else if (dims == 3)
    {
        ...
        {
            dim_n_for<2>(in_shape, in_index, out_index, concat_last_dim, std::true_type {});
        }
    }
    else if (dims == 4)
    {
        ...
        {
            dim_n_for<3>(in_shape, in_index, out_index, concat_last_dim, std::true_type {});
        }
    }
    return ok();
}
...
result<void> optimized::concat(datatype_t type, gsl::span<const gsl::byte *const> inputs, gsl::byte *output, const dims_t &out_shape,
    gsl::span<const dims_t> in_strides, const dims_t &out_strides, size_t axis, const dims_t &concat_dims, kernel_context &context) noexcept
{
    dims_t in_shape(out_shape);
