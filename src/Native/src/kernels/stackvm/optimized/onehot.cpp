...
result<void> one_hot_impl(const IndicesT *indices, T *output, const dims_t &indices_shape, const dims_t &out_shape,
    NNCASE_UNUSED const dims_t &out_strides, NNCASE_UNUSED size_t depth, T off_value, T on_value,
...
result<void> optimized::one_hot(datatype_t type, datatype_t indices_type, const gsl::byte *indices, gsl::byte *output, const dims_t &indices_shape, const dims_t &out_shape,
    const dims_t &out_strides, size_t depth, gsl::byte *values, size_t axis, runtime::stackvm::one_hot_mode_t mode, kernel_context &context) noexcept
{
