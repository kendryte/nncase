...
static int verify_shape(gsl::span<const size_t> in_a_shape,
   ...
   if (a_len < b_len) {
       return verify_shape_impl(in_b_shape, in_a_shape);
   }
...
static int verify_shape(gsl::span<const size_t> in_a_shape,
   ...
   {
       // printf(" no support two-away broadcast ... \n");
       // print_runtime_shape(in_a_shape, "-----in_a_shape");
       // print_runtime_shape(in_b_shape, "-----in_b_shape");
...
int optimized_binary_impl(
   ...
   if (verify_shape(in_a_shape, in_b_shape, len_a, len_b, out_shape))
   {
       return -1;
   }
   if (len_a >= len_b) {
