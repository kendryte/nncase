...
result<void> matmul_unit_impl(const float *input_a, const float *input_b, float *output,
    ...
    for (int32_t m = 0; m < M; m++) {
        for (int32_t n = 0; n < N; n++) {
            const float *ptr_a = &input_a[m * K];
            const float *ptr_b = &input_b[n];
            auto vec_s = vfmv_v_f_f32m1(0, vlmax);
            auto vec_zero = vfmv_v_f_f32m1(0, vlmax);
            int k = K;
            for (size_t vl = 0; k > 0; k -= vl) {
                vl = vsetvl_e32m1(k);
                auto vec_a = vle32_v_f32m1(ptr_a, vl);
                auto vec_b = vlse32_v_f32m1(ptr_b, N * sizeof(float), vl);
                ptr_a += vl;
                ptr_b += vl * N;
            ...
            output[m * N + n]= vfmv_f_s_f32m1_f32(vec_sum);
        }
    }
