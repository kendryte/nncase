...
#if __riscv_vector
static void reduce_block(int dim, int block, const float* input, float* out, int gap)
{
        __asm volatile(
                ...
                "vfdiv.vf v8, v8, ft0;"
                "vse32.v v8, ( %[out]);"
                ::[dim]"r"(dim), [block]"r"(block), [input]"r"(input), 
                [out]"r"(out), [gap]"r"(gap)
...
static void reduce_mean(const float* input, float* out, int dim, int n)
{
        __asm volatile(
                ...
                "addi a2, a2, 4;"
                "addi a3,a3, -1;"
                "bnez a3, reduce_mean_n_cycle%=;"
                ::[dim]"r"(dim), [n]"r"(n), [input]"r"(input), [out]"r"(out)
                :"t0","t1","a0","a1","a2", "a3","ft0","ft1","v8","v16"
        );
}

static void reduce_mean_s(int32_t c, int32_t dim, const float* input, float* out, int32_t gap)
{
#define BLOCK_N 32
        while(c --)
...
static int compute_size_by_index(gsl::span<const size_t> input, int start_index, int end_index)
{
        int init_value = 1;
...
static int get_parameter(gsl::span<const size_t> in_shape, gsl::span<const size_t> axis, gsl::span<int> out)
{
