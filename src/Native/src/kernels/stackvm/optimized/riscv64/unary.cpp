...
result<void> optimized_unary_impl(const float *input, float *output, const dims_t &shape) noexcept
{
    Top op;
...
//result<void> optimized::unary(runtime::stackvm::unary_op_t op, const float *input, float *output, const dims_t &shape,
//    const dims_t &in_strides, const dims_t &out_strides, kernel_context &context) noexcept
result<void>
optimized::unary(typecode_t dtype, runtime::stackvm::unary_op_t op, const gsl::byte *in,
...
#endif
    return stackvm::reference::unary(dtype, op, in, out, shape, in_strides, out_shape, out_strides, context);
}