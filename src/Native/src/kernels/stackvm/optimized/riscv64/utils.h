...
#define _RVV_FLOAT32_EXP_OP(LMUL, MLEN)                                        \
    static inline vfloat32m##LMUL##_t exp_ps(vfloat32m##LMUL##_t x,            \
        ...
        fx = vfmacc_vf_f32m##LMUL(vfmv_v_f_f32m##LMUL(0.5f, vl),               \
        ...
        vfloat32m##LMUL##_t pow2n =                                            \
            vreinterpret_v_i32m##LMUL##_f32m##LMUL(mm);                        \
                                                                               \
...
#define c_cephes_exp_C2_half (__float16_t) - 2.12194440e-4f

#define c_cephes_exp_p0_half (__float16_t)1.9875691500E-4f
#define c_cephes_exp_p1_half (__float16_t)1.3981999507E-3f
#define c_cephes_exp_p2_half (__float16_t)8.3334519073E-3f
#define c_cephes_exp_p3_half (__float16_t)4.1665795894E-2f
#define c_cephes_exp_p4_half (__float16_t)1.6666665459E-1f
#define c_cephes_exp_p5_half (__float16_t)5.0000001201E-1f
...
#define _RVV_FLOAT16_EXP_OP(LMUL, MLEN)                                        \
    static inline vfloat16m##LMUL##_t exp_ph(vfloat16m##LMUL##_t x,            \
        ...
        x = vfmax_vf_f16m##LMUL(x, c_exp_lo_half, vl);                         \
                                                                               \
        fx = vfmacc_vf_f16m##LMUL(vfmv_v_f_f16m##LMUL((__float16_t)0.5f, vl),  \
        ...
        fx = vfsub_vf_f16m##LMUL##_m(mask, tmp, tmp, (__float16_t)1.0f, vl);   \
                                                                               \
        ...
        y = vfadd_vf_f16m##LMUL(y, (__float16_t)1.0f, vl);                     \
                                                                               \
...
_RVV_FLOAT16_EXP_OP(f2, 32)
_RVV_FLOAT16_EXP_OP(f4, 64)
...
#define _RVV_FLOAT32_SINCOS_OP(LMUL, MLEN)                                     \
    static inline void sincos_ps(vfloat32m##LMUL##_t x,                        \
        ...
        vbool##MLEN##_t poly_mask = vmsne_vx_u32m##LMUL##_b##MLEN(             \
            vand_vx_u32m##LMUL(emm2, 2, vl), 0, vl);                           \
                                                                               \
        ...
        sign_mask_sin =                                                        \
            vmxor_mm_b##MLEN(sign_mask_sin,                                    \
                             vmsne_vx_u32m##LMUL##_b##MLEN(                    \
                                 vand_vx_u32m##LMUL(emm2, 4, vl), 0, vl),      \
                             vl);                                              \
        sign_mask_cos = vmsne_vx_u32m##LMUL##_b##MLEN(                         \
            vand_vx_u32m##LMUL(vsub_vx_u32m##LMUL(emm2, 2, vl), 4, vl), 0,     \
            vl);                                                               \
                                                                               \
        ...
        *ysin = vmerge_vvm_f32m##LMUL(sign_mask_sin, ys,                       \
                                      vfneg_v_f32m##LMUL(ys, vl), vl);         \
        *ycos = vmerge_vvm_f32m##LMUL(sign_mask_cos,                           \
                                      vfneg_v_f32m##LMUL(yc, vl), yc, vl);     \
    }
...
#define _RVV_FLOAT32_TANH_OP(LMUL, MLEN)                                       \
    static inline vfloat32m##LMUL##_t tanh_ps(vfloat32m##LMUL##_t x,           \
        ...
        vbool##MLEN##_t mask_l =                                               \
            vmfge_vf_f32m##LMUL##_b##MLEN(x2, c_cephes_tanh_C1, vl);           \
        vbool##MLEN##_t mask_l2 =                                              \
            vmfgt_vf_f32m##LMUL##_b##MLEN(x2, c_cephes_HALFMAXLOGF, vl);       \
                                                                               \
        /* abs(x) >= 0.625 */                                                  \
        vfloat32m##LMUL##_t exp_x_x =                                          \
            exp_ps(vfadd_vv_f32m##LMUL(x, x, vl), vl);                         \
        vfloat32m##LMUL##_t y0 = vfrsub_vf_f32m##LMUL(                         \
            vfrdiv_vf_f32m##LMUL(vfadd_vf_f32m##LMUL(exp_x_x, 1.f, vl), 2.f,   \
                                 vl),                                          \
            1.f, vl);                                                          \
                                                                               \
        /* abs(x) < 0.625                */                                    \
        ...
        vfloat32m##LMUL##_t z = vfmul_vv_f32m##LMUL(x, x, vl);                 \
                                                                               \
        vfloat32m##LMUL##_t y = vfmul_vf_f32m##LMUL(z, c_cephes_tanh_p0, vl);  \
        y = vfadd_vf_f32m##LMUL(y, c_cephes_tanh_p1, vl);                      \
        y = vfmul_vv_f32m##LMUL(y, z, vl);                                     \
        y = vfadd_vf_f32m##LMUL(y, c_cephes_tanh_p2, vl);                      \
        y = vfmul_vv_f32m##LMUL(y, z, vl);                                     \
        y = vfadd_vf_f32m##LMUL(y, c_cephes_tanh_p3, vl);                      \
        y = vfmul_vv_f32m##LMUL(y, z, vl);                                     \
        y = vfadd_vf_f32m##LMUL(y, c_cephes_tanh_p4, vl);                      \
                                                                               \
        y = vfmul_vv_f32m##LMUL(y, z, vl);                                     \
        y = vfmul_vv_f32m##LMUL(y, x, vl);                                     \
        y = vfadd_vv_f32m##LMUL(y, x, vl);                                     \
                                                                               \
        /* abs(x) > HALFMAXLOGF */                                             \
        vfloat32m##LMUL##_t y1 =                                               \
            vfsgnj_vv_f32m##LMUL(vfmv_v_f_f32m##LMUL(1.f, vl), x, vl);         \
                                                                               \
