...
result<void> write_tensor_buffer(value_t value, std::ofstream &of)
{
   ...
   of.write(reinterpret_cast<const char *>(v_tensor_span.buffer().data()), v_tensor_span.buffer().size_bytes());
   return ok();
}
...
result<void> run_core(const std::string &arg_file_path)
{
   ...
   gsl::span<gsl::byte> input_pool_span = { reinterpret_cast<gsl::byte *>(input_pool.data()), input_pool.size() };
   ...
   for (int i = 0; i < input_nums; i++)
   {
       ...
       size_t start, size;
       arg_file >> start >> size;
       try_var(_, hrt::create((typecode_t)type_code, dims, input_pool_span.subspan(start, size), false));
       parameters.push_back(_.impl());
   }

   auto kmodel = read_file(kmodel_path);
   interpreter *interp = new interpreter();
   auto dump_path = std::filesystem::path(arg_file_path).parent_path().string();
   nncase_interp_set_dump_root(interp, dump_path.c_str());
   try_(interp->load_model({ reinterpret_cast<const gsl::byte *>(kmodel.data()), kmodel.size() }, false));
