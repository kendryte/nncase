...
#define _RVV_FLOAT16_LOG_OP(LMUL, MLEN, TLEN)                                                  \
    static inline vfloat##TLEN##m##LMUL##_t log_ps(vfloat##TLEN##m##LMUL##_t x,                \
        size_t vl)                                                                             \
    {                                                                                          \
        x = vfmax_vf_f##TLEN##m##LMUL(                                                         \
            x, 0.f, vl); /* force flush to zero on denormal values */                          \
        vbool##MLEN##_t invalid_mask = vmfle_vf_f##TLEN##m##LMUL##_b##MLEN(x, 0.f, vl);        \
                                                                                               \
        vint##TLEN##m##LMUL##_t ux = vreinterpret_v_f##TLEN##m##LMUL##_i##TLEN##m##LMUL(x);    \
                                                                                               \
        ...
        /*       x = x + x - 1.0;      */                                                      \
        /*     } else { x = x - 1.0; } */                                                      \
        vbool##MLEN##_t mask = vmflt_vf_f##TLEN##m##LMUL##_b##MLEN(x, c_cephes_SQRTHF, vl);    \
        ...
        vfloat##TLEN##m##LMUL##_t z = vfmul_vv_f##TLEN##m##LMUL(x, x, vl);                     \
                                                                                               \
        vfloat##TLEN##m##LMUL##_t y = vfmul_vf_f##TLEN##m##LMUL(x, c_cephes_log_p0, vl);       \
        ...
        y = vfmul_vv_f##TLEN##m##LMUL(y, z, vl);                                               \
                                                                                               \
        vfloat##TLEN##m##LMUL##_t tmp = vfmul_vf_f##TLEN##m##LMUL(e, c_cephes_log_q1, vl);     \
        ...
        /* negative arg will be NAN */                                                         \
        vuint##TLEN##m##LMUL##_t xtmp = vreinterpret_v_f##TLEN##m##LMUL##_u##TLEN##m##LMUL(x); \
        x = vreinterpret_v_u##TLEN##m##LMUL##_f##TLEN##m##LMUL(                                \
            vor_vx_u##TLEN##m##LMUL##_m(invalid_mask, xtmp, xtmp, 0x7e00, vl));                \
        return x;                                                                              \
    }
...
#define _RVV_FLOAT_EXP_OP(LMUL, MLEN, TLEN, E, M)                                                 \
    static inline vfloat##TLEN##m##LMUL##_t exp_ps(vfloat##TLEN##m##LMUL##_t x,                   \
        size_t vl)                                                                                \
    {                                                                                             \
        ...
        fx = vfmacc_vf_f##TLEN##m##LMUL(vfmv_v_f_f##TLEN##m##LMUL(0.5f, vl),                      \
            c_cephes_LOG2EF, x, vl);                                                              \
                                                                                                  \
        /* perform a floorf */                                                                    \
        tmp = vfcvt_f_x_v_f##TLEN##m##LMUL(vfcvt_x_f_v_i##TLEN##m##LMUL(fx, vl), vl);             \
                                                                                                  \
        /* if greater, substract 1 */                                                             \
        vbool##MLEN##_t mask = vmfgt_vv_f##TLEN##m##LMUL##_b##MLEN(tmp, fx, vl);                  \
        fx = vfsub_vf_f##TLEN##m##LMUL##_m(mask, tmp, tmp, 1.f, vl);                              \
                                                                                                  \
        tmp = vfmul_vf_f##TLEN##m##LMUL(fx, c_cephes_exp_C1, vl);                                 \
        vfloat##TLEN##m##LMUL##_t z = vfmul_vf_f##TLEN##m##LMUL(fx, c_cephes_exp_C2, vl);         \
        x = vfsub_vv_f##TLEN##m##LMUL(x, tmp, vl);                                                \
        x = vfsub_vv_f##TLEN##m##LMUL(x, z, vl);                                                  \
                                                                                                  \
        vfloat##TLEN##m##LMUL##_t y = vfmul_vf_f##TLEN##m##LMUL(x, c_cephes_exp_p0, vl);          \
        ...
        vfloat##TLEN##m##LMUL##_t pow2n = vreinterpret_v_i##TLEN##m##LMUL##_f##TLEN##m##LMUL(mm); \
                                                                                                  \
...
#define _RVV_FLOAT_SINCOS_OP(LMUL, MLEN, TLEN)                                             \
    ...
    {                                                                                      \
        ...
        y1 = vfsub_vv_f##TLEN##m##LMUL(y1, vfmul_vf_f##TLEN##m##LMUL(z, 0.5f, vl), vl);    \
        y2 = vfadd_vv_f##TLEN##m##LMUL(y2, x, vl);                                         \
        y1 = vfadd_vf_f##TLEN##m##LMUL(y1, 1.f, vl);                                       \
                                                                                           \
        /* select the correct result from the two polynoms */                              \
        vfloat##TLEN##m##LMUL##_t ys = vmerge_vvm_f##TLEN##m##LMUL(poly_mask, y2, y1, vl); \
        vfloat##TLEN##m##LMUL##_t yc = vmerge_vvm_f##TLEN##m##LMUL(poly_mask, y1, y2, vl); \
        *ysin = vmerge_vvm_f##TLEN##m##LMUL(sign_mask_sin, ys,                             \
            vfneg_v_f##TLEN##m##LMUL(ys, vl), vl);                                         \
        *ycos = vmerge_vvm_f##TLEN##m##LMUL(sign_mask_cos,                                 \
            vfneg_v_f##TLEN##m##LMUL(yc, vl), yc, vl);                                     \
    }
...
#define _RVV_FLOAT_SIN_OP(LMUL, MLEN, TLEN)                                     \
    static inline vfloat##TLEN##m##LMUL##_t sin_ps(vfloat##TLEN##m##LMUL##_t x, \
        size_t vl)                                                              \
    {                                                                           \
...
#define _RVV_FLOAT_COS_OP(LMUL, MLEN, TLEN)                                     \
    static inline vfloat##TLEN##m##LMUL##_t cos_ps(vfloat##TLEN##m##LMUL##_t x, \
        size_t vl)                                                              \
    {                                                                           \
        vfloat##TLEN##m##LMUL##_t ysin, ycos;                                   \
        sincos_ps(x, &ysin, &ycos, vl);                                         \
...
#define _RVV_FLOAT_TANH_OP(LMUL, MLEN, TLEN)                                                                  \
    static inline vfloat##TLEN##m##LMUL##_t tanh_ps(vfloat##TLEN##m##LMUL##_t x,                              \
        size_t vl)                                                                                            \
    {                                                                                                         \
        vfloat##TLEN##m##LMUL##_t x2 = vfsgnj_vf_f##TLEN##m##LMUL(x, 1.f, vl);                                \
                                                                                                              \
        vbool##MLEN##_t mask_l = vmfge_vf_f##TLEN##m##LMUL##_b##MLEN(x2, c_cephes_tanh_C1, vl);               \
        vbool##MLEN##_t mask_l2 = vmfgt_vf_f##TLEN##m##LMUL##_b##MLEN(x2, c_cephes_HALFMAXLOGF, vl);          \
                                                                                                              \
        /* abs(x) >= 0.625 */                                                                                 \
        vfloat##TLEN##m##LMUL##_t exp_x_x = exp_ps(vfadd_vv_f##TLEN##m##LMUL(x, x, vl), vl);                  \
        vfloat##TLEN##m##LMUL##_t y0 = vfrsub_vf_f##TLEN##m##LMUL(                                            \
            vfrdiv_vf_f##TLEN##m##LMUL(vfadd_vf_f##TLEN##m##LMUL(exp_x_x, 1.f, vl), 2.f,                      \
                vl),                                                                                          \
            1.f, vl);                                                                                         \
                                                                                                              \
        /* abs(x) < 0.625                */                                                                   \
        ...
        vfloat##TLEN##m##LMUL##_t z = vfmul_vv_f##TLEN##m##LMUL(x, x, vl);                                    \
                                                                                                              \
        vfloat##TLEN##m##LMUL##_t y = vfmul_vf_f##TLEN##m##LMUL(z, c_cephes_tanh_p0, vl);                     \
        ...
        y = vfadd_vv_f##TLEN##m##LMUL(y, x, vl);                                                              \
                                                                                                              \
        /* abs(x) > HALFMAXLOGF */                                                                            \
        vfloat##TLEN##m##LMUL##_t y1 = vfsgnj_vv_f##TLEN##m##LMUL(vfmv_v_f_f##TLEN##m##LMUL(1.f, vl), x, vl); \
                                                                                                              \
