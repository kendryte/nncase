...
template <>
struct op_reader<tensor_custom_call_op_t>
{
    tensor_custom_call_op_t operator()(NNCASE_UNUSED span_reader &reader) const
    {
        tensor_custom_call_op_t op;
        op.registered_name = reader.read_string();
        op.input_size = reader.read_unaligned<int32_t>();
        op.field_size = reader.read_unaligned<int32_t>();
        op.fields = reader.read_span(op.field_size);
        return op;
    }
};
...
public:
    ...
    virtual result<void> visit(NNCASE_UNUSED const tensor_custom_call_op_t &op) noexcept { return default_visit(op); }
