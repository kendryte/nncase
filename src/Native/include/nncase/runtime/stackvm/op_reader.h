...
template <> struct tensor_op_reader<tensor_function_t::grid_sample> {
    tensor_grid_sample_op_t
    operator()(NNCASE_UNUSED span_reader &reader) const {
        tensor_grid_sample_op_t op;
        op.align_corners = static_cast<grid_sample_align_corners_t>(
            reader.read_unaligned<uint8_t>());
        op.mode =
            static_cast<grid_sample_mode_t>(reader.read_unaligned<uint8_t>());
        op.padding_mode = static_cast<grid_sample_padding_mode_t>(
...
class NNCASE_API tensor_op_visitor {
  public:
    ...
    virtual result<void>
    visit(NNCASE_UNUSED const tensor_grid_sample_op_t &op) noexcept {
        return default_visit(tensor_function_t::grid_sample, &op);
    }
    virtual result<void>
