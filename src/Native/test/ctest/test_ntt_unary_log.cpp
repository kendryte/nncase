...
TEST(UnaryTestLog, vector) { TEST_VECTOR(float) }
...
template <typename T, size_t vl> void test_vector_ulp(double ulp_threshold) {
    ...
    std::unique_ptr<tensor_type> ntt_input(new tensor_type);
    NttTest::init_tensor(*ntt_input, static_cast<T>(1), static_cast<T>(50));
...
TEST(UnaryTestLogFloat, ulp_error) { TEST_VECTOR_ULP(float, 1.) }
