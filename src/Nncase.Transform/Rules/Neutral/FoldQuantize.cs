...
public sealed partial class FoldQuantDeQuant : IRewriteRule
{
    ...
    Expr? GetReplace(Expr input, QuantParam q_param, QuantParam deq_param, IR.Math.Dequantize dequantize)
    {
...
public sealed partial class FoldDeQuantQuant : IRewriteRule
{
    ...
    public IPattern Pattern { get; } = IsQuantize("quantize", "output", x => true,
                                        IsDequantize("deq", "", x => true,
                                          IsWildcard("input"),
                                          IsTensorConst("deq_param")),
                                        IsTensorConst("q_param"));

    Expr? GetReplace(Expr input, QuantParam q_param, QuantParam deq_param, IR.Math.Quantize quantize)
    {
