...
public sealed class TreeSolverInitializer : TreeSolverBase, ITreeNodeVisitor<TreeSolverInitializer.Context, TreeSolverInitializer.InitResult>
{
    ...
    public InitResult Visit(OpNode value, Context context)
    {
        ...
        var kernelInfo = new MicroKernelInfo(tileVars.Select(i => 1).ToArray(), tileVars.Select((_, i) => new ValueRange<long>(0, value.DomainBounds[i])).ToArray(), Array.Empty<MicroKernelBufferInfo>(), (_, b, _) => b.MakeIntConst(1));
