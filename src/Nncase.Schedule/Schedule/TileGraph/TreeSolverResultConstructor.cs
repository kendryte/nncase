...
public sealed class GraphSolverArgumentsCollector : ITileGraphVisitor<Unit, Unit>
{
    public GraphSolverArgumentsCollector()
    {
        Inputs = new();
        Outputs = new();
    }
    ...
    public Unit Visit(TileGraph value, Unit arg1)
    {
        ...
        {
            foreach (var item in value.Vertices.OfType<TileGraph>())
            {
                Visit(item, arg1);
            }
        }
...
public sealed class GraphSolverResultConstructor : GraphSolverBase, ITileGraphVisitor<GraphSolverResultConstructor.Context, Unit>
{
    ...
    public GraphSolverResultConstructor(TileGraph tree, long objectiveValue, Assignment solution, ArgumentsInfo argumentsInfo, Dictionary<int, Dictionary<TileGraph, Dictionary<(TileGraph Node, BufferIdentity Buffer), IntExpr>>> levelNodeBufferBoxs, Dictionary<int, Dictionary<TileGraph, Dictionary<(TileGraph Node, BufferIdentity Buffer), Tuple<int, int>>>> levelTreeBufferLifeness, Solver solver, Dictionary<OpNode, OpNodeInfo> primitiveBufferInfo, Dictionary<TileGraph, TileNodeInfo> levelBufferInfos, Dictionary<ITileableNode, DomainInfo> domainInfos, ITargetOptions targetOptions)
    ...
    public Dictionary<int, Dictionary<TileGraph, Dictionary<(TileGraph Node, BufferIdentity Buffer), IntExpr>>> LevelTreeBufferSizes { get; }

    public Dictionary<int, Dictionary<TileGraph, Dictionary<(TileGraph Node, BufferIdentity Buffer), ulong>>> LevelTreeBufferOffsets { get; }

    public Dictionary<int, Dictionary<TileGraph, Dictionary<(TileGraph Node, BufferIdentity Buffer), Tuple<int, int>>>> LevelTreeBufferLifeness { get; }

    public Unit Visit(TileGraph value, Context context)
    {
        ...
        // var (parentbuilder, partentOffsets) = (loopBuilders[^1], forwardOffsets[^1]);
        if (value.ClustersCount != 0)
        {
            foreach (var child in value.Clusters.OfType<TileGraph>())
            {
                var childBuilder = T.Sequential();
                Visit(child, new(childBuilder, forwardOffsets[^1]));
    ...
    public void ScheduleBuffers()
    {
        foreach (var (level, treeBufferSizes) in LevelTreeBufferSizes)
        {
            ...
            foreach (var (root, nodeBufferSizes) in treeBufferSizes)
            {
                ...
                var validKeys = new List<(TileGraph Node, BufferIdentity Buffer)>();
    ...
    public Call ConstructResult(string moduleKind, int itemNumber)
    {
        ...
        Visit(RootGraph, new(bodyBuilder, Array.Empty<Expr>()));
    ...
    {
        var cbid = bid;
        TileGraph? parentNode = node.GetParentTileableNode(RootGraph);
        while (parentNode is TileGraph parentTileNode)
        {
            ...
            parentNode = (TileGraph)parentTileNode.Parent!;
            cbid = pbid;
        }
    ...
    private ParentSubViewInfo GetParentSubViewInfo(int storeLevel, ITileableNode node, BufferIdentity bid, AffineMap map, Expr[] forwardOffsets, IntExpr[] shapeExprs)
    {
        ...
        else
        {
            ...
            else if (node is TileGraph tileNode)
            {
    ...
    /// </summary>
    private TIR.Buffer GetParentAllocateBuffer(int storeLevel, TileGraph node, BufferIdentity bid, int[] shape, out bool innerAllocated)
    {
        var expr = bid.Node.Grid.Buffers[bid.Index];
        var tensorType = GetBufferTensorType(expr);
