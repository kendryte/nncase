...
namespace codegen
{
    ...
    {
    public:
        codegen_context(std::ostream &output, const std::unordered_map<memory_type_t, scheduler::memory_allocator *> &allocators, const std::unordered_map<llir::output_connector *, scheduler::memory_allocation> &allocations);

        memory_range get_allocation(llir::input_connector &conn) const
        {
            return get_allocation(*conn.connection());
        }

        memory_range get_allocation(llir::output_connector &conn) const;
    ...
    using emitter_t = std::function<std::unique_ptr<node_body>(llir::node &node, codegen_context &context)>;

    void register_emitter(llir::node_opcode opcode, emitter_t emitter);
    void disable_emitter(llir::node_opcode opcode);
    void gencode(codegen_context &context, xtl::span<llir::node *> compute_sequence);
}
}
