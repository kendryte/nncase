...
compile_command::compile_command(lyra::cli &cli)
{
    cli.add_argument(lyra::command("compile", [this](const lyra::group &) { this->run(); })
                         ...
                         .add_argument(lyra::opt(use_mse_quant_w_).name("--use-mse-quant-w").optional().help("use mse to refine weights quantilization or not, default is " + std::to_string(use_mse_quant_w_)))
