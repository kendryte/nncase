...
public sealed class PagedAttentionEvaluator : ITypeInferencer<PagedAttention>, ICostEvaluator<PagedAttention>, IEvaluator<PagedAttention>
{
    ...
    private static OrtKISharp.Tensor RefPagedAttn(OrtKISharp.Tensor query, PagedAttentionKVCache cache, float scale, int layerId)
    {
        var outputs = new List<OrtKISharp.Tensor>();
        long queryStart = 0;
        for (int requestId = 0; requestId < cache.NumRequests; requestId++)
        {
            var seqLen = cache.SeqLens[requestId];
            var queryLen = cache.SeqLens[requestId] - cache.ContextLens[requestId];
            ...
            var k = GatherKV(AttentionCacheKind.Key, cache, requestId, layerId, seqLen); // [num_heads, seq_len, head_dim]
            ...
            var v = GatherKV(AttentionCacheKind.Value, cache, requestId, layerId, seqLen); // [num_heads, seq_len, head_dim]
    ...
    private static OrtKISharp.Tensor GatherKV(AttentionCacheKind cacheKind, PagedAttentionKVCache cache, int requestId, int layerId, long seqLen)
    {
        var caches = new List<OrtKISharp.Tensor>();
        var contextBlockIds = cache.GetContextBlockIds(requestId, layerId);
        var outputSlotIds = cache.GetOutputSlotIds(cacheKind, requestId, layerId);
        ...
        for (int i = 0; i < blocksCount; i++)
        {
            ...
            var block = cache.GetBlock(cacheKind, blockId);
        ...
        // output slots
        var outputSlotsCount = outputSlotIds.Dimensions[0];
        for (int i = 0; i < outputSlotsCount; i++)
        {
            var slotId = outputSlotIds[i];
            var slot = cache.GetSlot(cacheKind, slotId);
