...
bool tflite_detection_postprocess_transform::on_try_match(node &node, transform_context &context)
{
    ...
    {
        if(tdp->output_0().shape() == shape_t {1, (size_t)tdp->max_detections(),4})
            return false;
        context.inputs.emplace_back(&tdp->boxes());
        context.inputs.emplace_back(&tdp->scores());
        context.inputs.emplace_back(&tdp->anchors());

        context.outputs.emplace_back(&tdp->output_0());
        context.outputs.emplace_back(&tdp->output_1());
        context.outputs.emplace_back(&tdp->output_2());
        context.outputs.emplace_back(&tdp->output_3());
...
void tflite_detection_postprocess_transform::process(transform_context &context)
{
    ...
    auto output_0 = context.outputs[0]->connections();
    auto output_1 = context.outputs[1]->connections();
    auto output_2 = context.outputs[2]->connections();
    auto output_3 = context.outputs[3]->connections();
    ...
    auto new_output_node_0 = context.graph.emplace<output_node>(output_0[0]->type(), new_output_shape_0);
    auto new_output_node_1 = context.graph.emplace<output_node>(output_1[0]->type(), new_output_shape_1);
    auto new_output_node_2 = context.graph.emplace<output_node>(output_2[0]->type(), new_output_shape_2);
    auto new_output_node_3 = context.graph.emplace<output_node>(output_3[0]->type(), new_output_shape_3);
    new_output_node_0->name("output_0");
    new_output_node_1->name("output_1");
    new_output_node_2->name("output_2");    
    new_output_node_3->name("output_3");
    ...
    new_output_node_0->input().connect(new_tdp->output_0());
    new_output_node_1->input().connect(new_tdp->output_1());
    new_output_node_2->input().connect(new_tdp->output_2());
    new_output_node_3->input().connect(new_tdp->output_3());
