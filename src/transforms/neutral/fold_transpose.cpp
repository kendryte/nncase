...
{
    if (auto tp1 = node_cast<transpose>(node))
    {
...
void transpose_to_reshape_transform::process(transform_context &context)
{
    ...
    auto rshape = context.graph.emplace<bitcast>(output.type(), output.shape(), context.matched_nodes[0]->output_at(0).shape());
