...
void add_input_dequantize_transform::process(transform_context &context)
{
    ...
    auto deq = context.graph.emplace<dequantize>(new_in_node->output().type(), new_in_node->output().shape(), dt_float32, quant_param_t { 0, 1 });
...
void add_output_quantize_transform::process(transform_context &context)
{
    ...
    auto q = context.graph.emplace<quantize>(dt_float32, output.shape(), dt_uint8, quant_param_t { 0, 1 });
