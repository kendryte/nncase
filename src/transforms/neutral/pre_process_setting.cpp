...
{
    if (graph.inputs()[0]->opcode() == op_input_node)
    {
        auto old_inputs = graph.inputs()[0]->output().connections();
        shape_t new_shape, old_shape;
        if (input_layout_ == "NHWC")
        {
            new_shape = { size_t(graph.inputs()[0]->output().shape()[0]), size_t(input_shape_[0]), size_t(input_shape_[1]), size_t(input_shape_[2]) };
        }
        else
        {
            new_shape = { size_t(graph.inputs()[0]->output().shape()[0]), size_t(input_shape_[2]), size_t(input_shape_[0]), size_t(input_shape_[1]) };
        }
        ...
        /**
             ...
             **/
        if (!new_shape.empty() && graph.inputs()[0]->output().shape() != new_shape)
        {
            ...
            {
                model_h = graph.inputs()[0]->output().shape()[1];
                model_w = graph.inputs()[0]->output().shape()[2];
            }
            else
            {
                model_h = graph.inputs()[0]->output().shape()[2];
                model_w = graph.inputs()[0]->output().shape()[3];
            }
