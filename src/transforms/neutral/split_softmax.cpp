...
void split_softmax_transform::process(transform_context &context)
{
    ...
    axis_t axes { sm.axis() };
    auto rmax = context.graph.emplace<reduce>(reduce_max, input_shape, axes, std::numeric_limits<float>::lowest(), true);
    ...
    auto rsum = context.graph.emplace<reduce>(reduce_sum, exp->output().shape(), axes, 0.f, true);
