    ...
    namespace neutral
    {
        struct conv2d_options
        {
            ...
            void deserialize(runtime::span_reader &reader)
            {
                ...
                reader.read_span(weights, out_channels * in_shape[1] / groups * filter_h * filter_w);
                reader.read_span(bias, out_channels);
            }
        };

        runtime::kernel_call_result conv2d(conv2d_options &options, runtime::interpreter &interpreter, runtime::interpreter_step_t step)
        {
