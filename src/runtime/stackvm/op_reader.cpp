...
std::error_condition op_visitor::next() noexcept
{
    ...
    else
    {
        ...
        case opcode_t::LDARGA:
            return visit(op_reader<ldarga_op_t>()(reader_));
        case opcode_t::DUP:
            return visit(op_reader<dup_op_t>()(reader_));
        case opcode_t::POP:
            return visit(op_reader<pop_op_t>()(reader_));
        case opcode_t::COMP_SHAPE:
            return visit(op_reader<comp_shape_op_t>()(reader_));
        case opcode_t::COMP_PADDINGS:
            return visit(op_reader<comp_paddings_op_t>()(reader_));
        case opcode_t::NEG:
    ...
    return nncase_errc::stackvm_illegal_instruction;
}

std::error_condition op_visitor::visit() noexcept
{
    std::error_condition err;
    interrupted_ = false;

    while (!interrupted_ && !err)
        err = next();
    return err;
}
