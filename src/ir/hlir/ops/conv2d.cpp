...
void conv2d::compile(hlir_compile_context &context)
{
    auto l_c = context.graph.emplace<llir::conv2d>(input().shape(), weights(), bias(), groups(), padding_h(), padding_w(), stride_h(), stride_w(),
        dilation_h(), dilation_w(), fused_activation());
...
void quantized_conv2d::compile(hlir_compile_context &context)
{
    auto l_c = context.graph.emplace<llir::quantized_conv2d>(input().shape(), weights(), bias(), groups(), padding_h(), padding_w(), stride_h(), stride_w(),
        dilation_h(), dilation_w(), input_offset(), filter_offset(), output_mul(), output_shift(), output_offset());
    context.add_input(input(), l_c->input());
    context.add_output(output(), l_c->output());
}
