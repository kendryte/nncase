...
nncase::hlir::upsample::upsample(datatype_t dt, shape_t input_shape, shape_t scales_shape, const std::vector<float> &scales): _scales(scales) {
...
void nncase::hlir::upsample::compile(hlir_compile_context &context) {
    auto l_c = context.graph.emplace<llir::upsample>(input_at(0).type(), input_at(0).shape(), input_at(1).shape(), _scales);
    context.add_input(input_at(0), l_c->input_at(0));
    context.add_input(input_at(1), l_c->input_at(1));
