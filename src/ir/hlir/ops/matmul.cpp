...
void matmul::compile(hlir_compile_context &context)
{
    auto l_b = context.graph.emplace<llir::matmul>(input_a().shape(), input_b().shape(), bias(), fused_activation());
...
void quantized_matmul::compile(hlir_compile_context &context)
{
    auto l_b = context.graph.emplace<llir::quantized_matmul>(input_a().shape(), input_b().shape(), bias(), input_a_offset(), input_b_offset(),
        output_mul(), output_shift(), output_offset());
    context.add_input(input_a(), l_b->input_a());
    context.add_input(input_b(), l_b->input_b());
    context.add_output(output(), l_b->output());
}
