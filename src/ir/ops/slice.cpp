...
slice::slice(datatype_t type, shape_t input_shape, axis_t begin, axis_t end, axis_t strides, int32_t begin_mask, int32_t end_mask, int32_t ellipsis_mask, int32_t new_axis_mask)
    : begin_(normalize_strided_slice_begin(input_shape, begin, strides, begin_mask))
    , end_(normalize_strided_slice_end(input_shape, begin_, end, strides, end_mask))
...
slice::slice(datatype_t type, shape_t input_shape, axis_t begin, axis_t end)
    : slice(type, input_shape, begin, end, axis_t(begin.size(), 1), 0, 0, 0, 0)
{
}
