...
gru::gru(shape_t input_shape, shape_t w_shape, shape_t r_shape, shape_t b_shape, shape_t output_shape,
    shape_t output_h_shape, lstm_direction direction, std::string framework, bool LBR)
    : direction_(direction), framework_(framework), LBR_(LBR)
{
...
bool gru::properties_equal(node &other) const
{
    ...
    return direction() == r.direction() && framework() == r.framework() && LBR() == r.LBR();
}
