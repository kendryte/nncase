...
{
    if (hlir::get_windowed_output_size(output_shape[2] + padding_h_.sum(), weights_.shape()[2], stride_h_, dilation_h_, false) != input_shape[2]
        || hlir::get_windowed_output_size(output_shape[3] + padding_w_.sum(), weights_.shape()[3], stride_w_, dilation_w_, false) != input_shape[3])
