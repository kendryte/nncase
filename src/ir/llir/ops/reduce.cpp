...
reduce::reduce(reduce_op_t reduce_op, shape_t input_shape, axis_t axis, float init_value, bool keep_dims)
    : reduce_op_(reduce_op), keep_dims_(keep_dims), axis_(hlir::normalize_reduce_axis(input_shape, axis)), init_value_(init_value)
{
    add_input("input", dt_float32, input_shape);
    add_output("output", dt_float32, hlir::get_reduced_shape(input_shape, axis_, keep_dims_));
}
