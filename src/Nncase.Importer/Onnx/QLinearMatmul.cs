...
namespace Nncase.Importer
{
    public partial class OnnxImporter
    {
        ...
        {
            var (input_a, input_b) = GetInputExprs<Expr, Expr>(op, 0, 3);
            var aScale = GetInputExpr<Expr>(op, 1);
            var aZeroPoint = GetInputExpr<Expr>(op, 2);
            var bScale = GetInputExpr<Expr>(op, 4);
            var bZeroPoint = GetInputExpr<Expr>(op, 5);
