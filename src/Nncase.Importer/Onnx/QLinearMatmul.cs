...
namespace Nncase.Importer
{
    public partial class OnnxImporter
    {
        private Expr VisitQLinearMatMul(in NodeProto op)
        {
            ...
            return SetOutputsNames(Quantize(matmul, new QuantParam(((TensorConst)yZeroPoint).Value.ToScalar<int>(), ((TensorConst)yScale).Value.ToScalar<float>()), ((TensorConst)yZeroPoint).ValueType.DType), op);
        }
    }
}
