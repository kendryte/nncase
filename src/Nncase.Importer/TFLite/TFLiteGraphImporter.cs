...
/// TFLite importer.
/// </summary>
public sealed partial class TFLiteGraphImporter : BaseGraphImporter
{
    ...
    private readonly Dictionary<int, Expr> _outputTensors = new();

    /// <summary>
    /// Initializes a new instance of the <see cref="TFLiteGraphImporter"/> class.
    /// </summary>
    /// <param name="model">TFLite model.</param>
    /// <param name="graph">TFLite graph.</param>
    /// <param name="compileSession">Compile session.</param>
    /// <param name="module">IR module.</param>
    public TFLiteGraphImporter(tflite.Model model, tflite.SubGraph graph, CompileSession compileSession, IRModule module)
    ...
    public (IEnumerable<Var> Inputs, Dictionary<Var, Expr[]> VarMap) CreateModelInputs()
    {
        var graph = _model.Subgraphs(0)!.Value;
        var inputsCount = graph.InputsLength;
        var createdInputs = new Var[inputsCount];
        var dynVarMap = Enumerable
            .Range(0, inputsCount)
            .SelectMany(i =>
            {
                var tensor = graph.Tensors(graph.Inputs(i))!.Value;
        ...
        {
            var inputId = graph.Inputs(i);
            var tensor = graph.Tensors(inputId)!.Value;
    ...
    protected override Expr CreateOutputs()
    {
        ...
        return outputs.Length == 1 ? outputs[0] : new IR.Tuple(outputs);
    }
