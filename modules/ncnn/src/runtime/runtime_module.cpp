...
result<void> ncnn_runtime_module::initialize_before_functions(
...
kernels::kernel_context &ncnn_runtime_module::kernel_context() noexcept {
...
ncnn_runtime_module::create_function() noexcept {
    std::unique_ptr<runtime_function> mod(new (std::nothrow)
                                              ncnn_runtime_function(*this));
    if (mod)
...
nncase::runtime::ncnn::create_ncnn_runtime_module() {
