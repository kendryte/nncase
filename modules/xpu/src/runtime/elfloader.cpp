...
int elfloader::invoke_elf(hardware_context_mt *hw_ctx_impl,
    ...
    for (size_t i = 0; i < ctx_.ehdr.e_shnum; i++) {
        memcpy(&sym_shdr,
               (void *)((uintptr_t)ctx_.elf + ctx_.ehdr.e_shoff +
                        ctx_.ehdr.e_shentsize * i),
    ...
    memcpy(&str_shdr,
           (void *)((uintptr_t)ctx_.elf + ctx_.ehdr.e_shoff +
                    ctx_.ehdr.e_shentsize * sym_shdr.sh_link),
           ctx_.ehdr.e_shentsize);
    ...
    for (size_t i = 0; i < sym_num; i++) {
        memcpy(&symbol,
               (void *)((uintptr_t)ctx_.elf + sym_shdr.sh_offset +
                        sym_shdr.sh_entsize * i),
               sym_shdr.sh_entsize);
        if (!strncmp((char *)((uintptr_t)ctx_.elf + str_shdr.sh_offset +
                              symbol.st_name),
                     "__global_pointer$", 19)) {
            break;
        }
