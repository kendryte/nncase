...
namespace detail {
template <class T> struct pool_partial_type;

template <> struct pool_partial_type<uint8_t> { using type = uint32_t; };

template <> struct pool_partial_type<float> { using type = float; };

template <class T>
using pool_partial_type_t = typename pool_partial_type<T>::type;
} // namespace detail

result<void> kpu_upload(const uint8_t *src, uint8_t *dest,
                        const runtime::k210::kpu_shape_t &in_shape,
                        uint32_t dma_ch);

inline result<void> kpu_download(const uint8_t *src, uint8_t *dest,
                                 const runtime::k210::kpu_shape_t &in_shape) {
...
void kpu_conv2d(const uint8_t *input, int64_t *workspace, uint8_t *output,
                const uint8_t *weights, int32_t in_h, int32_t in_w,
                int32_t in_channels, int32_t out_channels, uint8_t pad_value,
                int32_t arg_x, int32_t shift_x, int32_t arg_w, int32_t shift_w,
                int64_t arg_add,
                const runtime::k210::kpu_batchnorm_segment *batchnorm,
                const runtime::k210::kpu_activation_table_t &activation) {
    ...
    // conv
    {
        ...
        for (int32_t og = 0; og < groups; og++) {
            const uint8_t *w_group_p =
                weights + (size_t)og * g_oc * g_ic * FilterSize * FilterSize;

            for (int32_t oc = 0; oc < g_oc; oc++) {
                const uint8_t *w_oc_p =
                    w_group_p + (size_t)oc * g_ic * FilterSize * FilterSize;
                ...
                for (int32_t oy = 0; oy < in_h; oy++) {
                    for (int32_t ox = 0; ox < in_w; ox++) {
                        ...
                        for (int32_t ic = 0; ic < g_ic; ic++) {
                            const uint8_t *in_c_p =
                                input + ((size_t)og * g_ic + ic) * in_h * in_w;
                            const uint8_t *w_ic_p =
                                w_oc_p + (size_t)ic * FilterSize * FilterSize;
                        ...
                        auto alu_out = value + (arg_x * sum_x >> shift_x) +
                                       (arg_w * sum_w >> shift_w) +
                                       arg_add * g_ic;
        ...
        for (int32_t oc = 0; oc < out_channels; oc++) {
            ...
            for (size_t i = 0; i < channel_size; i++) {
                ...
                auto &seg = *std::find_if(
                    activation.rbegin(), activation.rend(),
                    [value](const runtime::k210::kpu_activation_segment &seg) {
                        return value > seg.start_x;
                    });
                auto act_value =
                    runtime::carry_shift<int64_t, true>(
                        (value - seg.start_x) * seg.mul, seg.shift) +
                    seg.add;
                *out_it++ = (uint8_t)kernels::detail::clamp(
                    act_value, int64_t(0), int64_t(255));
            }
        }
    }
}
...
template <class T>
inline void kpu_pool2d(const T *input, T *output, int32_t in_h, int32_t in_w,
                       int32_t in_channels,
        ...
        for (int32_t oy = 0; oy < out_h; oy++) {
            for (int32_t ox = 0; ox < out_w; ox++) {
                ...
                case kpu_pool_mean_4_s4: {
                    for (int32_t ky = 0; ky < filter; ky++) {
                        for (int32_t kx = 0; kx < filter; kx++) {
                            const int32_t in_y = kernels::detail::clamp(
                                in_y_origin + ky, 0, in_h - 1);
                            const int32_t in_x = kernels::detail::clamp(
                                in_x_origin + kx, 0, in_w - 1);
...
template <bool IsDepthwise, int32_t FilterSize>
void fake_kpu_conv2d(const float *input, float *output, const float *weights,
                     const float *bias, int32_t in_h, int32_t in_w,
                ...
                for (int32_t ox = 0; ox < in_w; ox++) {
                    ...
                    for (int32_t ic = 0; ic < g_ic; ic++) {
                        ...
                        for (int32_t ky = filter_y_start; ky < filter_y_end;
                             ky++) {
                            for (int32_t kx = filter_x_start; kx < filter_x_end;
                                 kx++) {
                                const int32_t in_y = in_y_origin + ky;
                                const int32_t in_x = in_x_origin + kx;
                    ...
                    *output++ = kernels::detail::apply_activation(
