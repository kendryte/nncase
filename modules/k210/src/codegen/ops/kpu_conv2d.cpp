...
void k210_module_builder::emit(kpu_conv2d &node)
{
    ...
    else
    {
        ...
        auto size_limit = 30;
        auto one_load_channels = std::min(node.output_channels(), (int32_t)std::floor(size_limit * 1024.f / one_channel_size));
        one_load_kernel_size = one_channel_size * one_load_channels;
        load_times = (int32_t)std::ceil(node.output_channels() / (float)one_load_channels);
    ...
    layer.image_addr.data.image_dst_addr = allocation(node.kpu_output()).start / 64;
    ...
    layer.image_size.data.o_col_high = (uint16_t)(node.kpu_output().shape()[2] - 1);
    layer.image_size.data.o_row_wid = (uint16_t)(node.kpu_output().shape()[3] - 1);
    ...
    layer.kernel_calc_type_cfg.data.channel_switch_addr = (uint16_t)(in_layout.row_len * node.input().shape()[2]);
    ...
    layer.write_back_cfg.data.wb_channel_switch_addr = (uint16_t)(out_layout.row_len * node.kpu_output().shape()[2]);
    ...
    layer.dma_parameter.data.channel_byte_num = (uint16_t)(node.kpu_output().shape()[3] * node.kpu_output().shape()[2] - 1);
    layer.dma_parameter.data.dma_total_byte = (uint32_t)(node.kpu_output().shape()[3] * node.kpu_output().shape()[2] * node.kpu_output().shape()[1] - 1);
    ...
    options.main_mem_output = node.has_main_mem_output() ? allocation(node.main_mem_output()).runtime_type() : memory_range {};
