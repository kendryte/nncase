...
auto quantize_bn(quantizer &quantizer, fake_kpu_conv2d &conv, constant &bias, float sx, const std::vector<float> w_scales, [[maybe_unused]] const quant_param_t &yq_p)
{
...
void kpu_conv2d_transform::process(transform_context &context)
{
    ...
    auto yq_p = quantizer.get_quant_param(quantizer.get(old_conv.output()), 8, quantizer::quant_mode::unsigned_mode);
    auto zq_p = quantizer.get_quant_param(quantizer.get(*context.outputs[0]), 8, quantizer::quant_mode::unsigned_mode);
