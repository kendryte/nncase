...
/**
 * @brief add conv2d 1x1 before (top_left, top_right, bottom_left, bottom_right) 2x2 slice, for kpu compute.
 * 
 */
bool strided_slice_conv2d_pool::on_try_match(node &node, transform_context &context)
{
    if (node.runtime_opcode() == op_slice)
    {
        // || slc.begin() == axis_t { 0, 0, 1, 0 } || slc.begin() == axis_t { 0, 0, 1, 1 }
        auto &slc = static_cast<slice &>(node);
        if ((slc.strides() == axis_t { 1, 1, 2, 2 })
            && (slc.begin() == axis_t { 0, 0, 0, 0 } || slc.begin() == axis_t { 0, 0, 0, 1 })
            && slc.end() == axis_t { (int32_t)slc.input().shape()[0], (int32_t)slc.input().shape()[1], (int32_t)slc.input().shape()[2], (int32_t)slc.input().shape()[3] }
            && slc.begin_mask() == 0
            && slc.end_mask() == 0
            && slc.new_axis_mask() == 0)
...
void strided_slice_conv2d_pool::process(transform_context &context)
{
    ...
    std::vector<float> weights_value { 1.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 1.f };
    auto weights = context.graph.emplace<constant>(dt_float32, weights_shape, weights_value);
    weights->name(old_slice.name() + "/Pre_Conv/Weights");

    shape_t bias_shape { 3 };
    std::vector<float> bias_value(3, 0.f);
    auto bias = context.graph.emplace<constant>(dt_float32, bias_shape, bias_value);
    ...
    if (old_slice.begin() == axis_t { 0, 0, 0, 0 } || old_slice.begin() == axis_t { 0, 0, 0, 1 })
    {
    ...
    auto conv = context.graph.emplace<conv2d>(last_output.shape(), weights_shape, 1, padding_h, padding_w, stride_h, stride_w, 1, 1, value_range<float>::full());
