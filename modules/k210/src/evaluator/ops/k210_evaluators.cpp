...
{
    register_evaluator(op_k210_kpu_upload, [](ir::node &node, function_evaluate_context &context) {
        ...
        nncase::runtime::k210::kpu_shape_t kpu_in_shape { (uint32_t)in_shape[0], (uint32_t)in_shape[1], (uint32_t)in_shape[2], (uint32_t)in_shape[3] };

        [[maybe_unused]] auto ret = kernels::k210::kpu_upload(input_mem.data(), output_mem.data(), kpu_in_shape, 0);
        // kpu_upload(const uint8_t *src, uint8_t *dest, const runtime::k210::kpu_shape_t &in_shape, uint32_t dma_ch)
    });

    register_evaluator(op_k210_kpu_download, [](ir::node &node, function_evaluate_context &context) {
        ...
        [[maybe_unused]] auto ret = kernels::k210::kpu_download(input_mem.data(), output_mem.data(), kpu_in_shape);
    });
...
#define FAKE_KPU_CONV2D_IMPL(is_depthwise_val, filter_size_val)                                                                 \
    ...
    register_evaluator(op_k210_kpu_conv2d, [](ir::node &node, function_evaluate_context &context) {
        ...
        [[maybe_unused]] auto batch_norm_mem = batch_norm.buffer().as_span<uint64_t>();
        [[maybe_unused]] auto activation_mem = activation.buffer().as_span<uint8_t>();
        ...
        auto conv_out_fmap_size = runtime::compute_size(conv_out_shape);
        nncase::runtime::k210::kpu_shape_t kpu_in_shape { (uint32_t)in_shape[0], (uint32_t)in_shape[1], (uint32_t)in_shape[2], (uint32_t)in_shape[3] };
        nncase::runtime::k210::kpu_shape_t kpu_out_shape { (uint32_t)in_shape[0], (uint32_t)rnode.output_channels(), (uint32_t)out_shape[2], (uint32_t)out_shape[3] };

        auto conv_output_tmp = std::make_unique<uint8_t[]>(conv_out_fmap_size);
        auto batch = in_shape[0];
        auto in_size_per_batch = runtime::compute_size(in_shape) / batch;
        auto conv_output_tmp_size_per_batch = conv_out_fmap_size / batch;
        auto out_size_per_batch = runtime::compute_size(rnode.outputs()[0]->shape()) / batch;
        auto p_input = input_mem.data();
        auto p_conv_ouput_tmp = conv_output_tmp.get();
        auto p_output = output_mem.data();
        auto download_output_tmp = std::make_unique<uint8_t[]>(runtime::compute_size(in_shape));
        auto p_download_output_tmp = download_output_tmp.get();
        auto pool_output_tmp = std::make_unique<uint8_t[]>(runtime::compute_size(out_shape));
        auto p_pool_output_tmp = pool_output_tmp.get();

        const auto groups = rnode.is_depthwise() ? rnode.output_channels() : 1;
        const auto g_oc = rnode.is_depthwise() ? 1 : rnode.output_channels();

        [[maybe_unused]] auto ret_dl = kernels::k210::kpu_download(p_input, p_download_output_tmp, kpu_in_shape);

#define KPU_CONV2D_IMPL(is_depthwise_val, filter_size_val)                                                                                         \
    if (rnode.is_depthwise() == is_depthwise_val && runtime::k210::get_kpu_filter_size(rnode.filter_type()) == filter_size_val)                    \
    {                                                                                                                                              \
        kernels::k210::kpu_conv2d<is_depthwise_val, filter_size_val>(p_download_output_tmp, workspace.get(), p_conv_ouput_tmp, weights_mem.data(), \
            in_shape[2], in_shape[3], in_shape[1], rnode.output_channels(), pad_value, quant_args.arg_x, quant_args.shift_x,                       \
            quant_args.arg_w, quant_args.shift_w, quant_args.arg_add, &bn[0], act);                                                                \
    }

        auto workspace = std::make_unique<int64_t[]>(groups * g_oc * in_shape[2] * in_shape[3]);
        auto p_pool_output_tmp_base = p_pool_output_tmp;
        for (size_t n = 0; n < batch; n++)
        {
            KPU_CONV2D_IMPL(true, 1)
            else KPU_CONV2D_IMPL(true, 3) else KPU_CONV2D_IMPL(false, 1) else KPU_CONV2D_IMPL(false, 3)

                kernels::k210::kpu_pool2d(p_conv_ouput_tmp, p_pool_output_tmp, in_shape[2], in_shape[3], rnode.output_channels(),
                    rnode.pool_type());
        ...
        [[maybe_unused]] auto ret_ul = kernels::k210::kpu_upload(p_pool_output_tmp_base, p_output, kpu_out_shape, 0);
    });
}
