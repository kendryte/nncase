...
public sealed class TIRConvertVisitor : ExprVisitor<Unit, Unit>
{
    ...
    {
        _mainBody.Add(TIR.F.XPU.SwishB(input, ret, beta));
    }

    private void GenerateReshape(Buffer input, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.ReShape(input, ret));
    }

    private void GenerateConcat(Concat concat, Buffer[] inputs, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Concat(concat.Axis, inputs, ret));
    }
    ...
    {
        _mainBody.Add(TIR.F.XPU.Slice(input, output, begins, ends, axes, distributedType));
    }
    ...
    private void GenerateBoxing(IR.CPU.Boxing boxing, Buffer[] arguments, Buffer ret, Call expr)
    {
        switch (expr.Arguments[0].CheckedType, boxing.NewType)
        {
            case (TensorType, DistributedType distTensorType):
                {
                    _mainBody.Add(TIR.F.XPU.TDMALoad(ret, arguments[0], distTensorType.NdSBP, distTensorType.Placement));
                }

                break;
            case (DistributedType distTensorType, TensorType):
                {
                    _mainBody.Add(TIR.F.XPU.TDMAStore(arguments[0], ret, distTensorType.NdSBP, distTensorType.Placement));
                }
            ...
            case (DistributedType inType, DistributedType outType):
                {
                    ...
                    if (partialSumPos.Length > 0)
                    {
                        ...
                        _mainBody.Add(TIR.F.XPU.GatherReduceScatter(arguments[0], ret, partialSumPos, placement));
                    }
                    else
                    {
                        _mainBody.Add(TIR.F.XPU.TDMAStore(arguments[0], None.Default, inType.NdSBP, inType.Placement));
                        _mainBody.Add(TIR.F.XPU.TDMALoad(ret, None.Default, outType.NdSBP, outType.Placement));
                    }
                }
    ...
    private void GenerateUnary(string unaryOp, ReadOnlySpan<Buffer> arguments, Buffer ret)
    {
        ...
        _mainBody.Add(TIR.F.XPU.Unary(unaryOp, input, ret));
    }
    ...
    private void GenerateBinary(Binary binary, Buffer[] arguments, Buffer ret, Call expr)
    {
        ...
        _mainBody.Add(TIR.F.XPU.Binary(binary.BinaryOp, lhs, rhs, outtype, arguments[0], arguments[1], ret));
    }

    private void GenerateMatmul(MatMul matmul, Buffer[] arguments, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Matmul(arguments[0], arguments[1], ret));
    }

    private void GenerateLayerNorm(LayerNorm layerNorm, Buffer[] arguments, Buffer ret, DistributedType distributedType)
    {
        _mainBody.Add(TIR.F.XPU.LayerNorm(layerNorm.Axis, layerNorm.Epsilon, layerNorm.UseMean, arguments[0], arguments[1], arguments[2], ret, distributedType));
    }
    ...
    {
        _mainBody.Add(TIR.F.XPU.InstanceNorm(eps, arguments[0], arguments[1], arguments[2], ret, distributedType));
    }

    private void GenerateGather(Gather gahter, Buffer[] arguments, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Gather(gahter.Axis, arguments[0], arguments[1], ret));
    }

    private void GenerateSoftmax(Softmax softmax, int axis, Buffer[] arguments, Buffer ret, DistributedType distributedType)
    {
        _mainBody.Add(TIR.F.XPU.Softmax(axis, arguments[0], ret, distributedType));
    }

    private void GenerateTranspose(Transpose transpose, int[] perm, Buffer[] arguments, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Transpose(perm, arguments[0], ret));
    }
    ...
    {
        _mainBody.Add(TIR.F.XPU.Conv2D(arguments[0], arguments[1], arguments[2], ret, stride, padding, dilation, groups, fusedClamp, distributedType));
    }
    ...
    {
        _mainBody.Add(TIR.F.XPU.ReduceArg(arguments[0], ret, axis, keepdims, selectLastIndex, op, dataType));
    }
    ...
    {
        _mainBody.Add(TIR.F.XPU.Resize(arguments[0], ret, roi, newSize, cubicCoeffA, excludeOutside, extrapolationValue, resize.ResizeMode, resize.TransformationMode, resize.NearestMode, resize.IsTFResize));
    }

    private void GenerateCast(DataType dataType, CastMode castMode, ReadOnlySpan<Buffer> arguments, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Cast(arguments[0], ret, dataType, castMode));
    }

    private void GenerateExpand(int[] shape, DistributedType distributedType, ReadOnlySpan<Buffer> arguments, Buffer ret)
    {
        _mainBody.Add(TIR.F.XPU.Expand(shape, distributedType, arguments[0], ret));
    }

    private void GenerateClamp(ReadOnlySpan<Buffer> arguments, Buffer ret, float min, float max)
    {
        _mainBody.Add(TIR.F.XPU.Clamp(arguments[0], ret, min, max));
    }
