...
internal sealed class XPUSameInputFusionMergeRule : Mutators.SameInputFusionMergeRule
{
    public override string ModuleKind => XPUTarget.Kind;
...
internal sealed class XPUFusionGroupMutator<T> : Mutators.FusionGroupMutator
  where T : IFusionChecker
{
    private readonly TileOptions _tileOptions;

    public XPUFusionGroupMutator(
...
internal sealed class CheckedConvertMutator : ExprRewriter
{
    ...
    {
        if (expr is Fusion { ModuleKind: XPUTarget.Kind } fusion)
        {
            if (!_fusionConertedCache.TryGetValue(fusion, out _))
            {
                ...
                {
                    var converter = new SingleXPUFusionConverter(_tileOptions);
    ...
    {
        if (expr.Target is Fusion { ModuleKind: XPUTarget.Kind } fusion)
        {
