...
internal sealed class XPUFusionToTirPass : ModulePass
{
    private readonly TileOptions _tileOptions;

    public XPUFusionToTirPass(TileOptions tileOptions)
    {
        ...
        for (int i = 0; i < module.Functions.Count; i++)
        {
            if (module.Functions[i] is Function { ModuleKind: string kind } func && kind == Callable.StackVMModuleKind)
            {
                ...
                var post = (Function)rewriter.Rewrite(
                    func,
                    new Mutators.IMergeRewriteRule[] { new XPUSameInputFusionMergeRule() },
                    (rule, option) => new XPUFusionGroupMutator<MultiFusionChecker>(fusionCheckCache, _tileOptions, rule, option),
