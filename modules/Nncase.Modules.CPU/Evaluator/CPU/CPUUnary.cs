...
/// <summary>
/// Evaluator for <see cref="CPUUnary"/>.
/// </summary>
public class CPUUnaryEvaluator : IEvaluator<CPUUnary>, ITypeInferencer<CPUUnary>, ICostEvaluator<CPUUnary>, IOpPrinter<CPUUnary>
{
    /// <inheritdoc/>
    public IValue Visit(IEvaluateContext context, CPUUnary unary)
    {
        var input_tensor = context.GetArgumentValueAsTensor(unary, CPUUnary.Input);
        ...
        var input = context.GetOrtArgumentValue(unary, CPUUnary.Input);
    ...
    /// <inheritdoc/>
    public IRType Visit(ITypeInferenceContext context, CPUUnary target)
    {
        var input = context.CheckArgumentType<TensorType>(target, CPUUnary.Input);
        return Visit(input);
    }

    /// <inheritdoc/>
    public Cost Visit(ICostEvaluateContext context, CPUUnary target)
    {
        var inputType = context.GetArgumentType<TensorType>(target, CPUUnary.Input);
    ...
    /// <inheritdoc/>
    public string Visit(IIRPrinterContext context, CPUUnary target, bool iLmode)
    {
