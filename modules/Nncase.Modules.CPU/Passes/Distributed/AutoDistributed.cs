...
internal sealed class AutoDistributedRewriter : ExprVisitor<Dictionary<IRType, List<Expr>>, Unit>
{
    ...
    public void SingleNodeMemoryExtractConstrains(CpModel model, IReadOnlyDictionary<ENode, BoolVar> vars)
    {
        ...
        foreach (var k in distTypes)
        {
            if (TargetOptions.HierarchySizes.Length > 1)
            {
                ...
                if (k.Expr is Call { Target: IR.Distributed.Boxing boxing } call && boxing.NewType is DistributedType distributedType && call.Arguments[0].CheckedType is DistributedType inType && inType.NdSBP.Any(sbp => sbp is SBPPartialSum) && distributedType != call.Arguments[0].CheckedType)
                {
        ...
        if (ReferenceEquals(expr, VisitRoot))
        {
            ...
            foreach (var i in Enumerable.Range(0, expr.Fields.Length))
            {
                var boxings = Visit(expr.Fields[i]).Values.
                    Select(l => l.Select(e => e.CheckedType is DistributedType dt ? (dt.NdSBP.Any(s => s is SBPPartialSum) ? IR.F.Distributed.Boxing(IR.F.Distributed.Boxing(e, new DistributedType(dt.TensorType, dt.NdSBP.Select(s => s is SBPPartialSum ? SBP.B : s).ToArray(), dt.Placement)), dt.TensorType) : IR.F.Distributed.Boxing(e, dt.TensorType)) : e).ToArray()).
        ...
        if (!valid)
        {
            if (target is Reshape)
            {
                ...
                foreach (var boxing in Utilities.DistributedUtility.GetLeafCandidateNDSBPs(tensorType, inType.Placement).
                    Select(ndsbp => IR.F.Distributed.Boxing(args[0], new DistributedType(tensorType, ndsbp, inType.Placement), true)))
                {
            ...
            if (call.CheckedType is DistributedType distType)
            {
                ...
                var partialBoxings = Utilities.DistributedUtility.GetPartialCandidateNDSBPs(distType).
                    Select(ndsbp => (ndsbp, IR.F.Distributed.Boxing(call, distType with { NdSBP = ndsbp }))).Select(p =>
                    {
                        var lastSbp = p.ndsbp;
                        var reduced = p.Item2;
                        return Utilities.DistributedUtility.GetLeafCandidateNDSBPs(distType.TensorType, distType.Placement).Where(ndsbp => lastSbp != ndsbp).Select(ndsbp => IR.F.Distributed.Boxing(reduced, distType with { NdSBP = ndsbp })).ToArray();
    ...
    private IReadOnlyList<Expr> GetReBoxings(Expr expr)
    {
        ...
        for (int i = 0; i < type.Placement.Rank; i++)
        {
            ...
            {
                if (tensorType.Shape[axis] is { IsFixed: true, FixedValue: long s } && Utilities.DistributedUtility.IsDivideExactly(s, type.Placement.Hierarchy[i]))
                {
    ...
    private Expr InstertTerminator(Expr expr)
    {
        Expr CreateFinalBoxing(Expr e, DistributedType type)
        {
            ...
            {
                var boxingP2B = IR.F.Distributed.Boxing(e, new DistributedType(type.TensorType, type.NdSBP.Select(s => s is SBPPartialSum ? SBP.B : s).ToArray(), type.Placement));
