...
internal sealed class AutoDistributedRewriter : ExprVisitor<Dictionary<IRType, List<Expr>>, Unit>
{
        ...
        if (ReferenceEquals(expr, VisitRoot))
        {
            ...
            foreach (var i in Enumerable.Range(0, expr.Fields.Length))
            {
                var boxings = Visit(expr.Fields[i]).Values.
                    Select(l => l.Select(e => e.CheckedType is DistributedType dt ? (dt.NdSBP.Any(s => s is SBPPartial) ? IR.F.CPU.Boxing(IR.F.CPU.Boxing(e, new DistributedType(dt.TensorType, dt.NdSBP.Select(s => s is SBPPartial ? SBP.B : s).ToArray(), dt.Placement)), dt.TensorType) : IR.F.CPU.Boxing(e, dt.TensorType)) : e).ToArray()).
    ...
    protected override Dictionary<IRType, List<Expr>> VisitLeafCall(Call expr)
    {
        ...
        if (TargetOptions.DistributedSearchStrategy is AutoDistributedSearchStrategy.ExpandAll && isSupported)
        {
            ...
            if (call.CheckedType is DistributedType distType)
            {
                ...
                var partialBoxings = DistributedUtility.GetPartialCandidateNDSBPs(distType).
                    Select(ndsbp => (ndsbp, IR.F.CPU.Boxing(call, distType with { NdSBP = ndsbp }))).Select(p =>
                    {
                        var lastSbp = p.ndsbp;
                        var reduced = p.Item2;
                        return DistributedUtility.GetLeafCandidateNDSBPs(distType.TensorType, distType.Placement).Where(ndsbp => lastSbp != ndsbp).Select(ndsbp => IR.F.CPU.Boxing(reduced, distType with { NdSBP = ndsbp })).ToArray();
    ...
    private IReadOnlyList<Expr> GetReBoxings(Expr expr)
    {
        ...
        for (int i = 0; i < type.Placement.Rank; i++)
        {
            ...
            {
                if (tensorType.Shape[axis] is { IsFixed: true, FixedValue: int s } && DistributedUtility.IsDivideExactly(s, type.Placement.Hierarchy[i]))
                {
    ...
    private Expr InstertTerminator(Expr expr)
    {
        Expr CreateFinalBoxing(Expr e, DistributedType type)
        {
            ...
            {
                var boxingP2B = IR.F.CPU.Boxing(e, new DistributedType(type.TensorType, type.NdSBP.Select(s => s is SBPPartial ? SBP.B : s).ToArray(), type.Placement));
