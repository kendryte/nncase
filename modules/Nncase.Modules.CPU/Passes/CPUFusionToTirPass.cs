    ...
    protected override Task<IRModule> RunCoreAsync(IRModule module, RunPassContext options)
    {
        ...
        for (int i = 0; i < module.Functions.Count; i++)
        {
            if (module.Functions[i] is Fusion { ModuleKind: CPUTarget.Kind } fusion)
            {
                ...
                var visitor = new KernelToTIRVisitor(primBody, deviceFuncs, fusionCheckCache, new BufferSchedule.BufferScheduler(((CpuTargetOptions)_compileOptions.TargetCompileOptions).HierarchySizes[0]), new BufferSchedule.LifeTimeCollector());
