...
public sealed partial class VectorizeConcatPropagation : RewriteRule<Pattern>
{
    ...
    private Expr? GetReplace(Vectorize vectorize, Concat concat, Call caller, Call callee, IReadOnlyList<BaseExpr> tupleInputs)
    {
        ...
        var newInputs = tupleInputs.Select(input => caller.WithArguments([(Vectorize.Input, input)])).ToArray();
...
public sealed partial class ConcatDevectorizePropagation : RewriteRule<Pattern>
{
    ...
    IsConcat(
        ...
        IsTuple(null, IsVArgsRepeat("tupleInputs", exprs =>
        {
            ...
            {
                patterns[i] = IsAlt(
                    PatternMatch.F.Tensors.IsDevectorize($"devectorize_{i}", $"callee_{i}", _ => true, IsWildcard($"input_{i}")),
    ...
    private Expr? GetReplace(Concat concat, Call caller, IReadOnlyList<BaseExpr> tupleInputs, IMatchResult result)
    {
        var firstDevectorize = (from i in Enumerable.Range(0, tupleInputs.Count)
                           let devectorize = result.GetValueOrDefault($"devectorize_{i}") as Devectorize
        ...
        {
            var devectorize = result.GetValueOrDefault($"devectorize_{i}") as Devectorize;
            ...
            else
            {
                ...
                newInputs[i] = IR.F.Tensors.Vectorize(
                    newInputs[i],
        ...
        return IR.F.Tensors.Devectorize(caller.WithArguments([(Concat.Input, new IR.Tuple(newInputs))]), firstDevectorize.Lanes.ToArray(), firstDevectorize.Axes.ToArray());
    }
}
