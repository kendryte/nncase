...
template <class T> std::vector<T> read_binary_file(const char *file_name) {
...
auto load_bin_to_kmodel(char *file_path, interpreter &interp) {
    ...
    interp.load_model({(const gsl::byte *)model_data.get(), model_size})
        .expect("cannot load kmodel.");
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        ...
        std::cerr << argv[0]
                  << " <kmodel> <input_0.bin> <input_1.bin> ... <input_N.bin> "
                  << std::endl;
        std::cerr << argv[0]
                  << " <kmodel> <input_0.bin> <input_1.bin> ... <input_N.bin> "
                     "<output_0.bin> <output_1.bin> ... <output_N.bin>"
                  << std::endl;
        return -1;
    }
    ...
    for (size_t i = 2, j = 0; i < 2 + interp.inputs_size(); i++, j++) {
        ...
        auto tensor = hrt::create(desc.datatype, shape, hrt::pool_shared)
                          .expect("cannot create input tensor");
        interp.input_tensor(j, tensor).expect("cannot set input tensor");

        auto span = tensor.impl()
                        ->to_host()
                        .unwrap()
                        ->buffer()
                        .as_host()
    ...
    //     auto tensor = hrt::create(desc.datatype, shape,
    ...
    for (int i = 2 + interp.inputs_size(), j = 0; i < argc; i++, j++) {
        ...
        auto mapped_buf =
            std::move(hrt::map(out, map_access_t::map_read).unwrap());
        auto vec = read_binary_file<unsigned char>(argv[i]);
        switch (desc.datatype) {
        case dt_boolean:
        case dt_uint8: {
            cos = cosine((const uint8_t *)mapped_buf.buffer().data(),
                         (const uint8_t *)vec.data(),
                         vec.size() / sizeof(uint8_t));
            break;
        }
        case dt_int8: {
            cos =
                cosine((const int8_t *)mapped_buf.buffer().data(),
                       (const int8_t *)vec.data(), vec.size() / sizeof(int8_t));
            break;
        }
        case dt_float32: {
            cos = cosine((const float *)mapped_buf.buffer().data(),
                         (const float *)vec.data(), vec.size() / sizeof(float));
            break;
        }
        case dt_int32: {
            cos = cosine((const int32_t *)mapped_buf.buffer().data(),
                         (const int32_t *)vec.data(),
                         vec.size() / sizeof(int32_t));
            break;
        }
        case dt_int64: {
            cos = cosine((const int64_t *)mapped_buf.buffer().data(),
                         (const int64_t *)vec.data(),
                         vec.size() / sizeof(int64_t));
            break;
        }
        default: {
            std::cerr << "not supported data type: " << desc.datatype
                      << std::endl;
            std::abort();
        }
        }

        std::cout << "output " << j << " cosine similarity: " << cos
                  << std::endl;
    }
