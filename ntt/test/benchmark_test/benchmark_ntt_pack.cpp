...
void benchmark_ntt_vectorize(const std::string &mode, const size_t run_size) {
    ...
    auto ntt_output = ntt::make_tensor<ElementType>(
    ...
    for (size_t i = 0; i < warmup_size; i++) {
        ntt::pack(ntt_input, ntt_output, ntt::fixed_shape_v<vectorize_dims...>);
    ...
    auto element_size = ntt_output.size() * ElementType::size() / P;
