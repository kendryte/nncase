...
class UnaryTestGenerator(BaseTestGenerator):
    ...
    def generate_test_name(self, input_is_dynamic, output_is_dynamic,
    ...
    def generate_ntt_output_to_test(self, datatype, op_str,
        ...
        input_element_type = self.get_element_cpp_type(datatype.cpp_type, vec_rank, vec_param)
        tensor_init_lhs_code = self.generate_tensor_init(
            ...
            name_suffix = "", element_cpp_type=input_element_type, integer_only = False
        )
    ...
    def generate_test_case(
            ...
            vector_rank,
            input_continuity):
        test_name = self.generate_test_name(input_is_dynamic, output_is_dynamic,
                                             dims_spec, vector_rank, input_continuity)
        ...
        code.extend(self.generate_function_name(f"UnaryTest{op_str}", datatype.name_suffix, test_name))
        if vector_rank > 0:
...
def generate_tests_for_op(op_str, generator):
    for datatype in ALL_DATATYPES:
        ...
        filename = f"test_ntt_unary_{op_str}_{datatype.name_suffix.lower()}.cpp"
...
if __name__ == "__main__":
    ...
    generate_cmake_list(generated_directory, generated_filenames, "unary_test.cmake", "UNARY_TESTS")
