...
TEST(UnaryTestSinhhalf, fixed_fixed) {
    // init
...
TEST(UnaryTestSinhhalf, fixed_ranked) {
    // init
...
TEST(UnaryTestSinhhalf, ranked_ranked) {
    // init
...
TEST(UnaryTestSinhhalf, ranked_fixed) {
    // init
...
TEST(UnaryTestSinh, vector) { TEST_VECTOR(_Float16) }
...
template <typename T, size_t vl> void test_vector_ulp(double ulp_threshold) {
    ...
    nncase::ntt::apply(ntt_input->shape(), [&](auto index) {
        ...
        nncase::ntt::apply(input_element.shape(), [&](auto idx) {
            output_element(idx) = (T)std::sinh((float)input_element(idx));
        });
    });
...
// TEST(UnaryTestCoshhalf, ulp_error) { TEST_VECTOR_ULP(_Float16, 3.) }
