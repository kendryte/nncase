...
TEST(UnaryTestSqrt, vector) { TEST_VECTOR(_Float16) }
...
template <typename T, size_t vl> void test_vector_ulp(double ulp_threshold) {
    ...
    std::unique_ptr<tensor_type> ntt_input(new tensor_type);
    NttTest::init_tensor(*ntt_input, static_cast<T>(0), static_cast<T>(1000));
    ...
    nncase::ntt::apply(ntt_input->shape(), [&](auto index) {
        ...
        nncase::ntt::apply(input_element.shape(), [&](auto idx) {
            output_element(idx) = (T)std::sqrt((float)input_element(idx));
        });
    });
...
TEST(UnaryTestSqrtFloat, ulp_error) { TEST_VECTOR_ULP(_Float16, 0.) }
