...
template <typename T, size_t vl> void test_vector() {
    ...
    NttTest::init_tensor(ntt_input, static_cast<T>(-5.f), static_cast<T>(5.f));
...
TEST(UnaryTestTanh, vector) { TEST_VECTOR(_Float16) }
...
template <typename T, size_t vl> void test_vector_ulp(double ulp_threshold) {
    ...
    nncase::ntt::apply(ntt_input->shape(), [&](auto index) {
        ...
        nncase::ntt::apply(input_element.shape(), [&](auto idx) {
            output_element(idx) = (T)std::tanh((float)input_element(idx));
        });
    });
...
#if __riscv
// TEST(UnaryTestTanhFloat, ulp_error) { TEST_VECTOR_ULP(_Float16, 2.) }
#endif
