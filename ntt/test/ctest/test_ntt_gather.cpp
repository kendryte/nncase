...
TEST(GatherTestHalf, no_pack_dynamic_shape_int32) {

    int32_t index_array[] = {1, 0, 2};
    auto tb = ntt::make_tensor_view(std::span<int32_t, 3>(index_array, 3),
                                    ntt::fixed_shape_v<1, 3>);

    auto ta = ntt::make_tensor<half>(ntt::fixed_shape_v<4, 5, 8, 3>);
    auto tc = ntt::make_tensor<half>(ntt::fixed_shape_v<4, 1, 3, 8, 3>);
    auto td = ntt::make_tensor<half>(ntt::fixed_shape_v<4, 1, 3, 8, 3>);
    std::iota(ta.elements().begin(), ta.elements().end(), 0.f);

    auto shape = ntt::make_shape(4, 5, 8, 3);
    auto ta_dynamic =
        ntt::make_tensor_view(std::span<half, std::dynamic_extent>(
                                  ta.elements().data(), shape.length()),
                              shape);
    ntt::gather(ta_dynamic, tb, tc, 1_dim);
...
TEST(GatherTestHalf, no_pack_dynamic_shape_int64) {
...
TEST(GatherTestHalf, no_pack_index_int32) {
...
TEST(GatherTestHalf, no_pack_index_int64) {
...
TEST(GatherTestHalf, pack1d_dim0_contiguous) {
    ...
    auto ta = ntt::make_tensor<half>(ntt::fixed_shape_v<M, N>);
    auto tb = ntt::make_tensor<int64_t>(ntt::fixed_shape_v<1, M / Period>);
    auto pa =
        ntt::make_tensor<ntt::vector<half, P>>(ntt::fixed_shape_v<M, N / P>);
    auto pc = ntt::make_tensor<ntt::vector<half, P>>(
        ntt::fixed_shape_v<1, M / Period, N / P>);
    std::iota(ta.elements().begin(), ta.elements().end(), 0.f);
    std::iota(tb.elements().begin(), tb.elements().end(), 0);
    std::ranges::for_each(tb.elements(), [](int64_t &x) { x *= Period; });
    ...
    // compare
    auto pd = ntt::make_tensor<ntt::vector<half, P>>(
        ntt::fixed_shape_v<1, M / Period, N / P>);
...
TEST(GatherTestHalf, pack1d_dim0_no_contiguous) {
    constexpr size_t P = NTT_VLEN / (sizeof(half) * 8);
...
TEST(GatherTestHalf, pack1d_dim1_contiguous) {
    constexpr size_t P = NTT_VLEN / (sizeof(half) * 8);
...
TEST(GatherTestHalf, pack1d_dim1_no_contiguous) {
    constexpr size_t P = NTT_VLEN / (sizeof(half) * 8);
