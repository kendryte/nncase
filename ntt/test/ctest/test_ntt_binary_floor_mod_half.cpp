...
TEST(BinaryTestFloorModInt32, fixed_fixed_fixed_broadcast_lhs_scalar) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::fixed_shape<1>>;
    std::unique_ptr<tensor_type1> ntt_lhs(new tensor_type1);
    NttTest::init_tensor(*ntt_lhs, -100, 100);

    using tensor_type2 = ntt::tensor<int16_t, ntt::fixed_shape<1, 3, 16, 16>>;
...
TEST(BinaryTestFloorModInt32, fixed_fixed_fixed_broadcast_rhs_scalar) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::fixed_shape<1, 3, 16, 16>>;
    std::unique_ptr<tensor_type1> ntt_lhs(new tensor_type1);
    NttTest::init_tensor(*ntt_lhs, -100, 100);

    using tensor_type2 = ntt::tensor<int16_t, ntt::fixed_shape<1>>;
    std::unique_ptr<tensor_type2> ntt_rhs(new tensor_type2);
    NttTest::init_tensor(*ntt_rhs, 1, 10);
...
TEST(BinaryTestFloorModInt32, fixed_fixed_fixed_broadcast_lhs_vector) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::fixed_shape<16>>;
    std::unique_ptr<tensor_type1> ntt_lhs(new tensor_type1);
    NttTest::init_tensor(*ntt_lhs, -100, 100);
...
TEST(BinaryTestFloorModInt32, fixed_fixed_fixed_broadcast_rhs_vector) {
    ...
    using tensor_type2 = ntt::tensor<int16_t, ntt::fixed_shape<16>>;
    std::unique_ptr<tensor_type2> ntt_rhs(new tensor_type2);
    NttTest::init_tensor(*ntt_rhs, 1, 10);
...
TEST(BinaryTestFloorModInt32, fixed_fixed_fixed_broadcast_multidirectional) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::fixed_shape<1, 3, 1, 16>>;
    std::unique_ptr<tensor_type1> ntt_lhs(new tensor_type1);
    NttTest::init_tensor(*ntt_lhs, -100, 100);

    using tensor_type2 = ntt::tensor<int16_t, ntt::fixed_shape<3, 1, 16, 1>>;
    std::unique_ptr<tensor_type2> ntt_rhs(new tensor_type2);
    NttTest::init_tensor(*ntt_rhs, 1, 10);

    // ntt
    using tensor_type3 = ntt::tensor<int16_t, ntt::fixed_shape<3, 3, 16, 16>>;
...
TEST(BinaryTestFloorModInt32, fixed_ranked_ranked) {
    ...
    using tensor_type2 = ntt::tensor<int16_t, ntt::ranked_shape<4>>;
...
TEST(BinaryTestFloorModInt32, fixed_ranked_ranked_broadcast_rhs_scalar) {
    ...
    using tensor_type2 = ntt::tensor<int16_t, ntt::ranked_shape<1>>;
    auto shape1 = ntt::make_ranked_shape(1);
    std::unique_ptr<tensor_type2> ntt_rhs(new tensor_type2(shape1));
    NttTest::init_tensor(*ntt_rhs, -10, -1);

    // ntt
    using tensor_type3 = ntt::tensor<int16_t, ntt::ranked_shape<4>>;
...
TEST(BinaryTestFloorModInt32, ranked_fixed_ranked) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::ranked_shape<4>>;
    auto shape = ntt::make_ranked_shape(1, 3, 16, 16);
...
TEST(BinaryTestFloorModInt32, ranked_fixed_ranked_broadcast_lhs_scalar) {
    // init
    using tensor_type1 = ntt::tensor<int16_t, ntt::ranked_shape<1>>;
    auto shape1 = ntt::make_ranked_shape(1);
...
TEST(BinaryTestFloorModInt32, ranked_ranked_ranked) {
    // init
    using tensor_type = ntt::tensor<int16_t, ntt::ranked_shape<4>>;
    auto shape = ntt::make_ranked_shape(1, 3, 16, 16);
...
template <typename T, size_t vl> void test_vector(T low, T high) {
    ntt::vector<T, vl> ntt_lhs, ntt_rhs;
    NttTest::init_tensor(ntt_lhs, static_cast<T>(-100), static_cast<T>(100));
    NttTest::init_tensor(ntt_rhs, low, high);
...
#define _TEST_VECTOR(T, lmul, low, high)                                       \
    test_vector<T, (NTT_VLEN) / (sizeof(T) * 8) * lmul>(low, high);

#define TEST_VECTOR(T, low, high)                                              \
    _TEST_VECTOR(T, 1, low, high)                                              \
    _TEST_VECTOR(T, 2, low, high)                                              \
    _TEST_VECTOR(T, 4, low, high)                                              \
    _TEST_VECTOR(T, 8, low, high)

TEST(UnaryTestFloorMod, vector) {
    TEST_VECTOR(int16_t, -10, -1)
    TEST_VECTOR(int16_t, 1, 10)
    TEST_VECTOR(int32_t, -10, -1)
    TEST_VECTOR(int32_t, 1, 10)
    TEST_VECTOR(int64_t, -10, -1)
    TEST_VECTOR(int64_t, 1, 10)
}
