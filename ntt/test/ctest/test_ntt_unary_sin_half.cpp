...
TEST(UnaryTestSinhalf, fixed_fixed) {
    // init
...
TEST(UnaryTestSinhalf, fixed_ranked) {
    // init
...
TEST(UnaryTestSinhalf, ranked_ranked) {
    // init
...
TEST(UnaryTestSinhalf, ranked_fixed) {
    // init
...
TEST(UnaryTestSin, vector) { TEST_VECTOR(_Float16) }
...
template <typename T, size_t vl> void test_vector_ulp(double ulp_threshold) {
    ...
    nncase::ntt::apply(ntt_input->shape(), [&](auto index) {
        ...
        nncase::ntt::apply(input_element.shape(), [&](auto idx) {
            output_element(idx) = (T)std::sin((float)input_element(idx));
        });
    });
...
#if __riscv
// TEST(UnaryTestSinhalf, ulp_error) { TEST_VECTOR_ULP(_Float16, 2.) }
#endif
