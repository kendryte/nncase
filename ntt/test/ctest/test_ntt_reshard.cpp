...
// auto global_local_data_ptr = nncase::ntt::make_tensor<nncase::ntt::vector<uintptr_t, 2>>(nncase::ntt::distributed::topology_shape);
// auto global_local_rdata_ptr = nncase::ntt::make_tensor<nncase::ntt::vector<uintptr_t, 2>>(nncase::ntt::distributed::topology_shape);
...
//         global_local_data_ptr(index)[0] = (uintptr_t)(nncase::ntt::runtime::thread_alloc(M * N * sizeof(float), 8));
//         global_local_data_ptr(index)[1] = global_local_data_ptr(index)[0] + M * N * sizeof(float);
//     });
...
//             mesh_index[1] = bid;
//             mesh_index[2] = tid;
//             auto local_data_lhs = reinterpret_cast<float *>(global_local_data_ptr(mesh_index)[0]);
//             auto local_data_rhs = reinterpret_cast<float*>(nncase::ntt::runtime::thread_alloc(M * N * sizeof(float), 8));

//             // shard(broadcast)
//             std::span<float, M * N> buffer(p_in, M * N);
//             tensor_view<float, fixed_shape_v<M, N>> tv(buffer);
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, I, I>> stv_src(std::span<float, M * N>(local_data_lhs, M * N));
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, I, I>> stv_dst(std::span<float, M * N>(local_data_rhs, M * N));
...
//             tensor_view<float, fixed_shape_v<M, N>, fixed_strides<N, 1>> tv(std::span<float, M * N>(p_in, M * N));
//             using sharding_type = sharding<mesh<topology::thread, cdims, bdims, tdims>, B, S<1, 2>, I>;
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding_type> stv_src(std::span<float, M * N / split_num>(local_data_lhs, M * N / split_num));
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding_type> stv_dst(std::span<float, M * N / split_num>(local_data_rhs, M * N / split_num));
...
//             // shard
//             tensor_view<float, fixed_shape_v<M, N>> tv_in(std::span<float, M * N>(p_in, M * N));
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, I, I>> stv_src(std::span<float, M * N>(local_data_lhs, M * N));
...
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, S<0, 1, 2>, I>> stv_dst(std::span<float, M * N / split_num>(local_data_rhs, M * N / split_num));
//             reshard(stv_src, stv_dst);

//             // unshard
//             tensor_view<float, fixed_shape_v<M, N>> tv_out(std::span<float, M * N>(p_out, M * N));
...
//             // shard(slice)
//             constexpr size_t split_num = cdims * bdims * tdims;
//             tensor_view<float, fixed_shape_v<M, N>> tv(std::span<float, M * N>(p_in, M * N));
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, S<0, 1, 2>, I>> stv_src(std::span<float, M * N / split_num>(local_data_lhs, M * N / split_num));
//             sharded_tensor_view<float, fixed_shape_v<M, N>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, I, I>> stv_dst(std::span<float, M * N>(local_data_rhs, M * N));
...
TEST(CpuTest, reshard_2D_different_sharding_spec_different_split_axis) {
    ...
    ntt::apply(ntt::distributed::detail::global_local_data_ptr.shape(), [&](auto index) {
        ntt::distributed::detail::global_local_data_ptr(index)(0_dim) = (uintptr_t)(ntt::runtime::thread_alloc(M * N * sizeof(float), 8));
        ntt::distributed::detail::global_local_data_ptr(index)(1_dim) = ntt::distributed::detail::global_local_data_ptr(index)(0_dim) + M * N * sizeof(float);
    });
    ...
    for (size_t id = 0; id < num; id++) {
        threads.emplace_back([cdims, bdims, tdims, id, M, N, p_in, p_out] {
            ...
            // shard(slice)
            constexpr size_t split_num = bdims * tdims;
            auto tv_in = ntt::make_tensor_view_from_address(p_in, ntt::fixed_shape_v<M, N>);
            auto tv_out = ntt::make_tensor_view_from_address(p_out, ntt::fixed_shape_v<M, N>);

            const auto program_ids = make_shape(cid, bid, tid);
            float *local_data_lhs = reinterpret_cast<float *>(ntt::distributed::detail::global_local_data_ptr(program_ids)(0_dim));
            auto local_data_rhs = reinterpret_cast<float *>(nncase::ntt::runtime::thread_alloc(M * N * sizeof(float), 8));

            using mesh_type = ntt::distributed::mesh<ntt::distributed::topology::thread, cdims, bdims, tdims>;
            auto sharding_src = ntt::distributed::make_sharding<mesh_type>(ntt::distributed::shard_policy::S<1, 2>(), ntt::distributed::shard_policy::B);
            auto sharding_dst = ntt::distributed::make_sharding<mesh_type>(ntt::distributed::shard_policy::B, ntt::distributed::shard_policy::S<1, 2>());
    ...
    ntt::apply(ntt::distributed::detail::global_local_data_ptr.shape(), [&](auto index) {
        thread_free((void *)ntt::distributed::detail::global_local_data_ptr(index)(0_dim));
    });
}
...
//     using tensor_type_lhs = ntt::tensor<float, ntt::fixed_shape_v<M_LHS, N_LHS>>;
//     std::unique_ptr<tensor_type_lhs> ntt_input(new tensor_type_lhs);

//     using tensor_type_rhs = ntt::tensor<float, ntt::fixed_shape_v<M_RHS, N_RHS, K_RHS>>;
//     std::unique_ptr<tensor_type_rhs> ntt_output(new tensor_type_rhs);
//     NttTest::init_tensor(*ntt_input, -2.f, 2.f);
//     NttTest::init_tensor(*ntt_output, -2.f, 2.f);
//     auto p_in = reinterpret_cast<float *>(ntt_input->elements().data());
//     auto p_out = reinterpret_cast<float *>(ntt_output->elements().data());
...
//     std::vector<std::thread> threads;
//     for (size_t id = 0; id < num; id++) {
//         threads.emplace_back([cdims, bdims, tdims, id, M_LHS, N_LHS, M_RHS, N_RHS, K_RHS, p_in, p_out] {
//             size_t cid = id / (bdims * tdims);
//             size_t bid = id % (bdims * tdims) / tdims;
//             size_t tid = id % (bdims * tdims) % tdims;
// #ifdef __APPLE__
...
//             // shard(slice)
//             constexpr size_t split_num = bdims * tdims;
//             tensor_view<float, fixed_shape_v<M_LHS, N_LHS>> tv_in(std::span<float, M_LHS * N_LHS>(p_in, M_LHS * N_LHS));
//             sharded_tensor_view<float, fixed_shape_v<M_LHS, N_LHS>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, S<1, 2>, I>> stv_src(std::span<float, M_LHS * N_LHS / split_num>(local_data, M_LHS * N_LHS / split_num));
//             sharded_tensor_view<float, fixed_shape_v<M_RHS, N_RHS, K_RHS>, sharding<mesh<topology::thread, cdims, bdims, tdims>, B, I, I, S<1, 2>>> stv_dst(std::span<float, M_RHS * N_RHS * K_RHS / split_num>(local_data + M_LHS * N_LHS, M_LHS * N_LHS / split_num));
//             tensor_view<float, fixed_shape_v<M_RHS, N_RHS, K_RHS>> tv_out(std::span<float, M_RHS * N_RHS * K_RHS>(p_out, M_RHS * N_RHS * K_RHS));

//             reshard(tv_in, stv_src);
//             reshard(stv_src, stv_dst);
//             reshard(stv_dst, tv_out);
//             auto reshaped_out = tv_out.reshape(fixed_shape_v<M_LHS, N_LHS>{});
