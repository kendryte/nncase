...
class timer_record : public nncase::ntt::runtime::timer_record_base<record_id> {
  public:
    __device__ bool is_valid() const override {
        return instance_id_.cid != -1 && instance_id_.bid != -1 &&
               instance_id_.tid != -1;
    }

    __device__ void set_time(std::string_view function_name,
    ...
    __device__ void set_level(std::string_view filename,
                              profiling_level level) override {
        // auto &stats = function_stats_[filename];
        // stats.level = level;
    }

    // print statistics
    __device__ void console_print() const override {
    ...
    __device__ void csv_print(std::string_view filename) const override {
        // if (is_valid()) {
        ...
        //     csv_file
        //         << "Core Id,Block Id,Thread Id,Function,Level,Calls,Total
        //         Time "
    ...
    __device__ void markdown_print(std::string_view filename) const override {
    ...
    __device__ void json_print(std::string_view filename) const override {
    ...
    __device__ timer_record() = default;

    __device__ ~timer_record() {
        // console_print();
    ...
    __device__ void set_id(record_id id) override { instance_id_ = id; }
};
...
struct cuda_thread_context_t {
    ...
    __device__ static cuda_thread_context_t &current() noexcept;
};
