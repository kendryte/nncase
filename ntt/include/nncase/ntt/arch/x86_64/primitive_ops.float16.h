...
template <> struct cast_elem<ntt::vector<half, 16>, float> {
    ...
    operator()(const ntt::vector<half, 16> &v) const noexcept {
        ...
        __m256 fp32_v0 = _mm256_cvtph_ps(m128_ptr[0]);
        __m256 fp32_v1 = _mm256_cvtph_ps(m128_ptr[1]);
        return std::array<ntt::vector<float, 8>, 2>{fp32_v0, fp32_v1};
    }
};

// cast_elem
template <> struct cast_elem<ntt::vector<float, 2, 8>, half> {
    ntt::vector<half, 16>
    operator()(const ntt::vector<float, 2, 8> &v) const noexcept {
        __m128i fp16_v0 = _mm256_cvtps_ph(v(0_dim), _MM_FROUND_TO_NEAREST_INT);
        __m128i fp16_v1 = _mm256_cvtps_ph(v(1_dim), _MM_FROUND_TO_NEAREST_INT);
        __m256i packed = _mm256_setr_m128i(fp16_v0, fp16_v1);
        return packed;
    }
};
...
template <> struct mul_add<half, ntt::vector<half, 16>, ntt::vector<half, 16>> {
    ...
    operator()(const half &f1, const ntt::vector<half, 16> &v2,
        ...
        return ntt::cast_elem<half>(v3_fp32);
    }
};

template <>
struct mul_add<half, ntt::vector<half, 16>, ntt::vector<float, 2, 8>> {
    ntt::vector<float, 2, 8>
    operator()(const half &f1, const ntt::vector<half, 16> &v2,
