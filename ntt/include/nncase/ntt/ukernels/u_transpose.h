...
class u_transpose_impl<TIn, TOut, fixed_shape_t<2, 0, 3, 1>> {

  public:
    constexpr void operator()(const TIn &input, TOut &output,
                              const fixed_shape_t<2, 0, 3, 1> &) {
...
template <Tensor TIn, class TOut>
class u_transpose_impl<TIn, TOut, fixed_shape_t<2, 1, 3, 0>> {

  public:
    constexpr void operator()(const TIn &input, TOut &output,
                              const fixed_shape_t<2, 1, 3, 0> &) {
        for (auto i = 0; i < input.shape()[0]; i++) {
            for (auto j = 0; j < input.shape()[1]; j++) {
                for (auto k = 0; k < input.shape()[2]; k++) {
                    for (auto l = 0; l < input.shape()[3]; l++) {
                        output(k, j, l, i) = input(i, j, k, l);
                    }
                }
            }
        }
