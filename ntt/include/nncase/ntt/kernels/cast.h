        ...
        if constexpr (scale >= 1.f) {
            ntt::apply(output.shape(), [&](auto index) {
                ...
                ntt::u_cast<in_offset_scale, out_offset_scale>(
                    &input(in_index), packedAxes.rank() == 1 ? input.strides()[fixed_dim_v<packedAxes.at(0)>] : 1, &output(index), 1, 1);
            });
        } else {
            ntt::apply(input.shape(), [&](auto index) {
                ...
                if constexpr (packedAxes.rank() == 1)
                    out_index[fixed_dim_v<packedAxes.at(0)>] *=
                        out_offset_scale;
                ntt::u_cast<in_offset_scale, out_offset_scale>(
                    &input(index), 1, &output(out_index), packedAxes.rank() == 1 ? output.strides()[fixed_dim_v<packedAxes.at(0)>] : 1, 1);
            });
        }
#endif
    }
