...
struct tensor_reduce_arg_op_t
{
    ...
    explicit tensor_reduce_arg_op_t(datatype_t datatype, uint8_t rshape_src, uint8_t rstride_src, uint8_t rstride_dest, reduce_arg_op_t reduce_arg_op, uint8_t rshape_axis, bool keep_dims, bool select_last_idx) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::REDUCEARG), datatype(datatype), rshape_src(rshape_src), rstride_src(rstride_src), rstride_dest(rstride_dest), reduce_arg_op(reduce_arg_op), rshape_axis(rshape_axis), keep_dims(keep_dims), select_last_idx(select_last_idx)
    {
    }
};
