...
struct tensor_conv2d_op_t
{
    ...
    explicit tensor_conv2d_op_t(uint8_t rshape_src, uint8_t rshape_kernel, uint16_t groups, uint16_t stride_h, uint16_t stride_w, uint16_t dilation_h, uint16_t dilation_w, float fused_clamp_low, float fused_clamp_high) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::CONV2D), rshape_src(rshape_src), rshape_kernel(rshape_kernel), groups(groups), stride_h(stride_h), stride_w(stride_w), dilation_h(dilation_h), dilation_w(dilation_w), fused_clamp_low(fused_clamp_low), fused_clamp_high(fused_clamp_high)
    {
    }
};
...
struct tensor_reduce_op_t
{
    ...
    explicit tensor_reduce_op_t(uint8_t rshape_src, reduce_op_t reduce_op, uint8_t rshape_axis) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::REDUCE), rshape_src(rshape_src), reduce_op(reduce_op), rshape_axis(rshape_axis)
    {
    }
};
...
struct tensor_binary_op_t
{
    ...
    explicit tensor_binary_op_t(uint8_t rshape_src1, uint8_t rshape_src2, binary_op_t binary_op, float fused_clamp_low, float fused_clamp_high) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::BINARY), rshape_src1(rshape_src1), rshape_src2(rshape_src2), binary_op(binary_op), fused_clamp_low(fused_clamp_low), fused_clamp_high(fused_clamp_high)
    {
    }
};
...
struct tensor_unary_op_t
{
    ...
    explicit tensor_unary_op_t(uint8_t rshape_src, unary_op_t unary_op) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::UNARY), rshape_src(rshape_src), unary_op(unary_op)
    {
    }
};
