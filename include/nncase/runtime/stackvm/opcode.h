...
enum class tensor_function_t
{
    ...
    REDUCE = 0x0016,
    REDUCEARG = 0x0017,
    REDUCE_WINDOW2D = 0x0018,
    RESIZE_IMAGE = 0x0019,
    SLICE = 0x001A,
    SOFTMAX = 0x001B,
    SPACE_TO_BATCH = 0x001C,
    TAKE = 0x001D,
    TRANSPOSE = 0x001E,
    UNARY = 0x001F,
};
...
struct tensor_reduce_arg_op_t
{
    ...
    explicit tensor_reduce_arg_op_t(datatype_t datatype_src, uint8_t rshape_src, uint8_t rstride_src, datatype_t datatype_dest, uint8_t rstride_dest, reduce_arg_op_t reduce_arg_op, uint8_t rshape_axis, bool keep_dims, bool select_last_idx) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::REDUCEARG), datatype_src(datatype_src), rshape_src(rshape_src), rstride_src(rstride_src), datatype_dest(datatype_dest), rstride_dest(rstride_dest), reduce_arg_op(reduce_arg_op), rshape_axis(rshape_axis), keep_dims(keep_dims), select_last_idx(select_last_idx)
    {
    }
};
