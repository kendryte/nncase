...
struct tensor_random_normal_op_t
{
    ...
    explicit tensor_random_normal_op_t(datatype_t datatype_dest, uint8_t rshape_dst, float mean, float std, float seed) noexcept
        : opcode(opcode_t::TENSOR), funct(tensor_function_t::RANDOM_NORMAL), datatype_dest(datatype_dest), rshape_dst(rshape_dst), mean(mean), std(std), seed(seed)
    {
    }
};
