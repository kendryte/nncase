...
class TriluTest : public KernelTest,
...
TEST_P(TriluTest, trilu) {
    ...
    int64_t a_ptr[] = {alpha};
    auto a = hrt::create(nncase::dt_int64, {1},
                         {reinterpret_cast<gsl::byte *>(a_ptr), sizeof(a_ptr)},
                         true, host_runtime_tensor::pool_cpu_only)
                 .expect("create tensor failed");

    //    todo ort no implement Trilu
    // expected
    //    auto output_ort = ortki_Trilu(l_ort, runtime_tensor_2_ort_tensor(a),
    //    0); size_t size = 0; void *ptr_ort = tensor_buffer(output_ort, &size);
    ...
    auto output = kernels::stackvm::trilu(input.impl(), a.impl(), upper.impl())
                      .expect("trilu failed");
    runtime_tensor actual(output.as<tensor>().expect("as tensor failed"));

    auto output1 = kernels::stackvm::trilu(input.impl(), a.impl(), upper.impl())
                       .expect("trilu failed");
