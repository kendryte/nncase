...
TEST_P(ReduceMaxTest, ReduceMax) {
    ...
    auto select_last_idx = hrt::create(dt_int64, {1},
                                       ...
                                       true, host_runtime_tensor::pool_cpu_only)
                               .expect("create tensor failed");
    auto output_ort = ortki_ReduceMean(runtime_tensor_2_ort_tensor(a), 0, 0, 0);
    ...
    // actual
    auto output =
        kernels::stackvm::reduce(runtime::stackvm::reduce_op_t::mean, a.impl(), axis.impl(), keepDims.impl(), select_last_idx.impl())
