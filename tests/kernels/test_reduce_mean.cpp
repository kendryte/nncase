...
TEST_P(ReduceMeanTest, ReduceMean) {
    ...
    if (axis_arry1.size() == 2 && a.shape().size() >= 2) {
        ...
        auto axis1 = hrt::create(dt_int64, {2},
                                 ...
                                 true, host_runtime_tensor::pool_cpu_only)
                         .expect("create tensor failed");
        auto output_ort1 = ortki_ReduceMean(runtime_tensor_2_ort_tensor(a),
    ...
    if (axis_arry1.size() == 3 && a.shape().size() >= 3) {
        ...
        auto axis2 = hrt::create(dt_int64, {3},
                                 ...
                                 true, host_runtime_tensor::pool_cpu_only)
                         .expect("create tensor failed");
        auto output_ort2 = ortki_ReduceMean(runtime_tensor_2_ort_tensor(a),
    ...
    if (axis_arry1.size() == 4 && a.shape().size() >= 4) {
        ...
        auto axis3 = hrt::create(dt_int64, {4},
                                 ...
                                 true, host_runtime_tensor::pool_cpu_only)
                         .expect("create tensor failed");
        auto output_ort3 = ortki_ReduceMean(runtime_tensor_2_ort_tensor(a),
