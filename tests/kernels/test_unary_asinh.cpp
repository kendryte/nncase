...
class UnaryTest
  ...
  public:
    void SetUp() override {
        ...
        float ptr_ort[] = {1.0f};
        input = hrt::create(typecode, i_shape,
                            {reinterpret_cast<gsl::byte *>(ptr_ort), 4}, true,
...
TEST_P(UnaryTest, asinh) {
    ...
    // expected
    auto output_ort = ortki_Asinh(orts[0]);
    size_t size = 0;
    ...
    auto output = kernels::stackvm::unary(
                      nncase::runtime::stackvm::unary_op_t::asinh, input.impl())
                      .expect("unary failed");
