...
a0 = hrt::create(typecode_0, shape_0, host_runtime_tensor::pool_cpu_only).expect("create tensor failed"); \
            INIT_TENSOR(a0, init_f0); \
a1 = hrt::create(typecode_1, shape_1, host_runtime_tensor::pool_cpu_only).expect("create tensor failed"); \
...
#define GET_ACTUAL_ARGS_2_ATTR_0(op_fn, op_name) \
        auto output = op_fn(op_name, a0.impl(), a1.impl()).expect(std::string(#op_fn).append(" failed"));  \
...
#define NNCASE_TEST_BODY_ARGS_2_ATTR_0(test_class, test_name, op_fn, sub_op_name, ort_op, out_compare_type) \
...
#define NNCASE_TESTSUITE_INIT_ARGS_2(test_class,test_name,type0,type1,shape0,shape1) \
