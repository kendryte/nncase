...
#define NNCASE_TEST_CLASS_ARGS_4(class_name)                                          \
    class class_name : public KernelTest,                                      \
      ...
      public:                                                                  \
        void SetUp() override {                                                \
            auto &&[typecode_a, typecode_b, typecode_c, typecode_d,     \
                shape_a, shape_b, shape_c, shape_d] = GetParam();                  \
...
#define NNCASE_TEST_GENERATOR_8(a, b, c, d, e, f, g, h)    \
     testing::Combine(NNCASE_TEST_VALUE(a), \
                        NNCASE_TEST_VALUE(b), \
                        NNCASE_TEST_VALUE(c),  \
                        NNCASE_TEST_VALUE(d),  \
                        NNCASE_TEST_VALUE(e),  \
...
#define GET_DEFAULT_TEST_SHAPE() dims_t{1, 3, 16, 16}, dims_t{3, 16, 16}, \
    dims_t{3, 16, 1}, dims_t{16, 16}, \
    dims_t{16, 1}, dims_t{1, 16, 1}, \
    dims_t{16}, dims_t{1}, dims_t{}

#define GET_DEFAULT_TEST_VECTOR_SHAPE() dims_t{4}

#define GET_DEFAULT_TEST_SCALAR_SHAPE() dims_t{1}
...
#define NNCASE_CONDITION_GET(condition) \
    case dt_##condition: \
        arr[index] = static_cast<condition##_t>(get<condition##_t>(tensor, index)); \
        break; 
