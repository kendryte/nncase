...
TEST_P(BinaryTest, max) {
    OrtKITensor *orts[2];
    orts[0] = runtime_tensor_2_ort_tensor(lhs);
    orts[1] = runtime_tensor_2_ort_tensor(rhs);

    // expected
    auto output_ort = ortki_Max(orts, sizeof(orts) / sizeof(orts[0]));
    size_t size = 0;
    ...
    // actual
    auto output =
        kernels::stackvm::binary(nncase::runtime::stackvm::binary_op_t::max,
                                 lhs.impl(), rhs.impl())
