...
TEST_P(TopKTest, TopK) {
    ...
    auto k = hrt::create(lhs.datatype(), {1},
                         {reinterpret_cast<gsl::byte *>(k_array), size},
                         true, host_runtime_tensor::pool_cpu_only)
                 .expect("create tensor failed");
    auto output_ort = tensor_seq_get_value(ortki_TopK(l_ort, runtime_tensor_2_ort_tensor(k), -1, 1, 1), 0);
    ...
    auto largest = hrt::create(lhs.datatype(), {1},
                            {reinterpret_cast<gsl::byte *>(largest_array), size},
                            true, host_runtime_tensor::pool_cpu_only)
                    .expect("create tensor failed");
    int64_t sorted_array[] = {1};
    auto sorted = hrt::create(lhs.datatype(), {1},
                            {reinterpret_cast<gsl::byte *>(sorted_array), size},
                            true, host_runtime_tensor::pool_cpu_only)
                    .expect("create tensor failed");
    auto output =
        kernels::stackvm::top_k(lhs.impl(), k.impl(), axis.impl(), largest.impl(), sorted.impl())
            .expect("topk failed");
