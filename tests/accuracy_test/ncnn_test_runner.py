        ...
        with ncnn.Net() as net:
            ...
            for idx, value in enumerate(self.inputs):
                new_value = self.transform_input(
                    self.data_pre_process(value['data']), "float32", "CPU")
            ...
            for n in range(number):
                with net.create_extractor() as ex:
                    # set input
                    for idx, value in enumerate(self.inputs):
                        in_mat = ncnn.Mat(new_inputs[idx][n])
                        ex.input(value['name'], in_mat)

                    # get output
                    output = []
                    for idx, value in enumerate(self.outputs):
                        out_mat = ncnn.Mat()
                        ex.extract(value['name'], out_mat)
                        out_arr = np.array(out_mat)
                        out_mat.release()
                        output.append(out_arr)
                        if not test_utils.in_ci():
                            dump_bin_file(os.path.join(
                                self.case_dir, f'cpu_result_{idx}_{n}.bin'), out_arr)
                            dump_txt_file(os.path.join(
                                self.case_dir, f'cpu_result_{idx}_{n}.txt'), out_arr)
