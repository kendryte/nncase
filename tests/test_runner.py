        ...
        for input_idx, input in enumerate(inputs):
            ...
            for batch_idx in range(batch_number):
                ...
                if method == 'random':
                    ...
                    if name == 'calib':
                        if (input_idx == 0):
                            data = np.loadtxt(f'./qwen3_calib/input_ids{batch_idx:01d}.txt')
                        if (input_idx == 1):
                            data = np.loadtxt(f'./qwen3_calib/attention_mask{batch_idx:01d}.txt')
                        if (input_idx == 2):
                            data = np.loadtxt(f'./qwen3_calib/position_ids{batch_idx:01d}.txt')
                        if (input_idx == 3):
                            data = np.loadtxt(f'./qwen3_calib/past_key_values{batch_idx:01d}.txt')
                    else:
                        inputs_idx = 0
                        if (input_idx == 0):
                            data = np.loadtxt(f'./qwen3_calib/input_ids{inputs_idx:01d}.txt')
                        if (input_idx == 1):
                            data = np.loadtxt(f'./qwen3_calib/attention_mask{inputs_idx:01d}.txt')
                        if (input_idx == 2):
                            data = np.loadtxt(f'./qwen3_calib/position_ids{inputs_idx:01d}.txt')
                        if (input_idx == 3):
                            data = np.loadtxt(f'./qwen3_calib/past_key_values{inputs_idx:01d}.txt')
